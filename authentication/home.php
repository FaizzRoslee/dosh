	...
	<div class="col-xs-4"><?php 	
		...
		<div class="panel panel-default">
			...
			<table class="table">
                ...
                <!-- interface for announcement counts -->
                <?php			
                /*********/
                $newAnnouce_onClick = "redirectForm('".$sys_config['submission_path']."main.php?menu=17');";
                // sample sql
                // SELECT * FROM `tbl_announcement` WHERE (isDeleted=0 AND Active=1) AND (CURDATE() BETWEEN StartingDate AND EndingDate)
                $newAnnouce = _get_RowExist('tbl_Announcement',array('AND (isDeleted'=>'= 0', 'AND Active'=>'= 1)', 'AND (CURDATE()'=>'BETWEEN StartingDate AND EndingDate)'),'');
                /*********/
                ?>
                <tr>
                    <td><a href="#" onClick="<?= $newAnnouce_onClick ?>"><?= _LBL_MESSAGE ?> <span class="badge"><?= $newAnnouce; ?></span></a></td>
                </tr>
