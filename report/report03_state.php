...
?>
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		...
		?>
		<tr class="contents">
			<td colspan="<?= (12-$minus_1) ?>" class="label"><?= _get_StrFromCondition('sys_State','State_Name','State_ID',$value); ?></td>
		</tr>
		<?php
					//=====================================================
		...
		?>
		<tr class="contents">
			<td colspan="<?= (12-$minus_1) ?>" class="label">- <i><?= _LBL_SUPPLIER_NAME ?>:</i> <?= $cName; ?></td>
		</tr>
		<?php
								}
		?>
		<tr class="contents">
			...
			?>
			<td width="<?= $width_10_11 ?>" style="<?= $text_valign_mid ?>"><?= isset($arrayChem[$i]['Chemical_ID'])?nl2br(_get_StrFromCondition('tbl_Chemical','Chemical_Name','Chemical_ID',$arrayChem[$i]['Chemical_ID'])):''; ?></td>
		...
		<tr class="contents" height="30">
			<td colspan="<?= (12-$minus_1) ?>" <?= $td_mid_center; ?>><?= _LBL_NO_RECORD ?></td>
		</tr>
		<?php
					}
					$showExport += $bil;
	...
	<table width="100%">
		<tr>
			<td align="center"><?php
				if($showExport>0){
					$url_pdf = 'report03_to_pdf_state.php?rptType='.$rptType.'&lvlType='.$lvlType.'&chemType='.$chemType.'&status='.$status.'&rptYear='.$rptYear.'&state='.$state;
