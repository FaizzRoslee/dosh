<?php
	...
	func_header("",
				...
				$sys_config['includes_path']."jquery/jquery.ui.combobox/jquery.ui.combobox.css,". // css	
				"", // css	
				$sys_config['includes_path']."javascript/js_common.js,". // javascript
				$sys_config['includes_path']."jquery/jquery-ui-1.10.2.js,". // css	
				$sys_config['includes_path']."jquery/jquery.ui.combobox/jquery.ui.combobox.js,". // css	
				"", // javascript
				false, // menu
				false, // portlet
				false, // header
	...
	*/
	...
	if($rptType=="rpt02" && ( $lvlType!="" || $lvlType=="" ))
		$display = "";
	elseif(($rptType=="rpt01" || $rptType=="rpt06") && $lvlType!="")
		$display = "";
	else
		$display = "none";
	/*******/
...
?>
	...
	$(document).ready(function() {
		...
		$("#send").live("click",function(){
			...
			else if( $("#rptType").val()=="rpt02" ) 
				rptFile = "report02.php";
			else if( $("#rptType").val()=="rpt06" ) 
				rptFile = "report06_to_excel.php";
			
			if(rptFile!=''){
				var url 	= rptFile +"?rptType="+ $("#rptType").val() +"&lvlType="+ $("#lvlType").val() 
							+"&chemType="+ $("#chemType").val()
							+"&rptYear="+ $("#rptYear").val() +"&state="+ $("#selectTo").val() +"&comp="+ comp;
		...
		$('#selectTo option, #selectFrom option').removeAttr('title');
	});
	...
	(function($){
		...
		reloadComp = function(){
			...
			$.ajax({
				...
				success: function(msg){
					if (msg != ""){
						$("#comp").html(msg);
						if( $("#comp option:selected").length > 0 ){
							// alert( $("#comp option:selected").length +" "+ $("#comp option:selected").text() +" "+ $("#comp option:selected").val()  );
							var selectedText = $("#comp option:selected").text();
							$("#comp").combobox();
							$("#comp").closest("td").find(".ui-combobox-input").val( selectedText );
						}
					}
				}
			});
		...
		showHide = function(){
			...
			// alert(lvlType + ' ' + rptType);
			$("#tr_chemType, #tr_comp").hide();
			if(rptType=="rpt02" && ( lvlType=="" || lvlType!="" )){
				$("#send").show();
			}else if((rptType=="rpt01" || rptType=="rpt06") && lvlType!="" ){
				$("#send").show();
			}else{
				$("#send").hide();
			}
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
			...
			<td width="80%"><?php
				$param1 = 'rptType';
				$param2 = '';
				$param3 = array_merge($arrReport, array(array("value"=>"rpt06","label"=>_LBL_REPORT_OVERALL)));
		...
		<tr id="tr_comp" style="display:<?= $display2 ?>" class="contents">
			...
			<td><select id="comp" name="comp"></select></td>
		</td>
		<tr class="contents">
