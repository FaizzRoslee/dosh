<?php
	...
	// check if chemical type is substance and redirect to report01_all_to_excel_sub.php
	if($chemType==$arrChemicalType[0]['value'] || $chemType=='1')
	{
        // form redirect link
        $file = 'report01_all_to_excel_sub.php';
        $parameter  = 'rptType='  . $rptType;
        $parameter .= '&lvlType=' . $lvlType;
        $parameter .= '&chemType=' . $chemType;
        $parameter .= '&rptYear=' . $rptYear;
    ...
    foreach($arr_state as $value){
        ...
        // save row
        $rows_total_supply[] = $row; 
        // apply style to cell
        $cell = $first_element . $row .':'. $last_element . $row;
        ...
        merge_cells($objPHPExcel, $cell);
                        ...
                        $cell = $first_element . $row .':'. $last_element . ($row+1);
