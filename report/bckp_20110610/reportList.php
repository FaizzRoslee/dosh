<?php
	...
	if(isset($_POST['send'])){
		...
		//==========================================================
		$url = $fileRpt.'?rptType='.$rptType.'&lvlType='.$lvlType.'&chemType='.$chemType.'&rptYear='.$rptYear.'&state='.$state;
		echo '
...
?>
	...
	function showHide(){
		...
		var lvlType = d.getElementById('lvlType').value;
		d.getElementById('tr_chemType').style.display = 'none';
		if(rptType=='rpt02' && ( lvlType=='' || lvlType!='' )){
		...
		if(rptType=='rpt01')
			d.getElementById('tr_chemType').style.display = '';
	}
	</script>
		...
		<tr class="contents">
			...
			<td><?php
				if($userLevelID!=5){
					...
					echo '<input type="hidden" name="selectTo[]" value="'.$state.'">';
				}
			?></td>
		</tr>
		<tr class="contents">
