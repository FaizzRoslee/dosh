...
?>
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
			...
			<?php 
					$minus_sup = 1;
					if( ($lvlType!=7 || $lvlType=="all") && $userLevelID!=5){
						$minus_sup = 0;
			?>
			...
			<?php 
					...
					$minus_use = 1;
					if( ($lvlType!=6 || $lvlType=="all")  && $userLevelID!=5){
						$minus_use = 0;
			?>
	...
	<table width="100%">
		<tr>
			<td align="center"><?php
				if($showExport>0){
					$url_pdf = "report01_all_to_pdf.php?rptType=$rptType&lvlType=$lvlType&chemType=$chemType&rptYear=$rptYear&state=$state&comp=$comp";
...
// 				// updates debug
// 				if($showExport>0){
// 					$url_excel = "report01_all_to_excel_debug1.php?rptType=$rptType&lvlType=$lvlType&chemType=$chemType&rptYear=$rptYear&state=$state&comp=$comp";
// 					echo form_button('convert',_LBL_CONVERT_TO_EXCEL." Debug",array('type'=>'button','onClick'=>"funcPopup('".$url_excel."');"));
// 					echo '&nbsp;';
// 				}
				
				
// 				if($showExport>0){
// 					$url_word = "report01_all_to_word.php?rptType=$rptType&lvlType=$lvlType&chemType=$chemType&rptYear=$rptYear&state=$state&comp=$comp";
