<?php
	//****************************************************************/
	// filename: hazCheckList_get.php
	// description: get list of the hazard checking [physical,health,environmental]
	//***************************************************************/
	...
	$sIndexColumn = "Code_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( " ","hcod.CAS_No","ht.Type_Name","hs.Signal_Name","hcod.Active"," " );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);

	/* DB table to use */
	$sTable = "tbl_Hazard_Codes hcod";
	...
	* Joining
	*/
	$sJoin = " LEFT OUTER JOIN tbl_Hazard_Type ht ON hcod.Type_ID = ht.Type_ID";
	$sJoin .= " LEFT OUTER JOIN tbl_Hazard_Signal hs ON hcod.Signal_ID = hs.Signal_ID";
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND hcod.isDeleted = 0 ";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = "redirectForm('hazCheckView.php?codeID=$uniqueID')";
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					/*********/
					$imgParam1 = $sys_config["images_path"]."icons/edit.gif";
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = "redirectForm('hazCheckAddEdit.php?codeID=$uniqueID')";
					$activity .= "&nbsp;"._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					/*********/
