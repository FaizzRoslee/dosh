<?php
	...
	$sIndexColumn = "hc.Class_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( ' ','ht.Type_Name','hc.Class_Name','hc.Active',' ' );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);

	/* DB table to use */
	$sTable = "tbl_Hazard_Classification hc";
	...
	* Joining
	*/
	$sJoin = ' LEFT JOIN tbl_Hazard_Type ht ON hc.Type_ID = ht.Type_ID';
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = 'redirectForm(\'hazClassAddEdit.php?classID='.$uniqueID.'\')';
