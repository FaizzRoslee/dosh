<?php
	...
	if(isset($_POST["checkbox"]) && isset($_POST["reset"])){
		 ...
		 *
		 ************************/
		$dataColumns = "u.Usr_LoginID,uc.Contact_Email";
		$dataTable = "sys_User u";
		$dataJoin = array("sys_User_Client uc"=>"u.Usr_ID = uc.Usr_ID");
		$dataWhere = array("AND u.Usr_ID" => "IN (". implode(",",$_POST["checkbox"]) .")");
		$arrayData = _get_arrayData($dataColumns,$dataTable,$dataJoin,$dataWhere); 
		/*********/
		foreach($arrayData as $detail){
			$content = _get_Notification4($detail["Usr_LoginID"],$detail["Contact_Email"]); //4 is for registration
			if(is_array($content)){
	...
	$(document).ready(function() {
		...
		$("#resetpass").on("click",function(){
			...
			if( $("input[type=checkbox][value]:checked").length > 0 ){
				var r = confirm("<?= _LBL_PROCEED_RESET ?>");
				if(r==true){
					$(this).button("disable").find(".sp-load").show();
					$("#myForm").append('<input type="text" name="reset" />');
					$("#myForm").submit();
				}
			}
		});
				...
				<table style="width:100%">
					<tr>
						...
						<td width="60%" align="right"><?php
							...
							echo form_button("resetpass",_LBL_RESET_PASSWORD . $spLoad,$dOthers);
							/*********/
							echo "&nbsp;";
							/*********/
