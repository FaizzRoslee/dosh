<?php
	...
	elseif(isset($_GET["categoryID"])) $categoryID = $_GET["categoryID"];
	else $categoryID = 0;
	/*********/
	...
	/*********/
	if($categoryID!=0) $addEdit = _LBL_EDIT;
	...
	if(isset($_POST["save"])){
		...
		/*********/
		if($categoryID!=0){
			$sqlUpd	= "UPDATE tbl_Hazard_Category SET"
					. " Category_Name = ".quote_smart($categoryName).","
			...
			$sqlIns	= "INSERT INTO tbl_Hazard_Category ("
					. " Category_Name,Pictogram_ID,Signal_ID,"
					. " Statement_ID,Class_ID,Type_ID,"
					. " PreStatement_ID1,PreStatement_ID2,PreStatement_ID3,PreStatement_ID4,"
					. " Active,"
					. " RecordBy, RecordDate,"
					. " UpdateBy, UpdateDate"
					. " ) VALUES ("
					. " ".quote_smart($categoryName).",".quote_smart($pictogramID).",".quote_smart($signalID).","
					. " ".quote_smart($hStatementID).",".quote_smart($classID).",".quote_smart($typeID).","
			...
			$categoryID = $db->sql_nextid();
			/*********/
			$msg = ($resIns) ? "Insert Successfully" : "Insert Unsuccessfully";
			/*********/
		}
		/*********/
		func_add_audittrail($updBy,$msg." [".$categoryID."]","Admin-Hazard Category","tbl_Hazard_Category,".$categoryID);
		echo '
			<script language="Javascript">
			alert("'.$msg.'");
			location.href="hazCategoryList.php";
			</script>
			';
		exit();
	}
...
?>
	...
	<?php func_window_open2(_LBL_HAZARD_CATEGORY ." :: ". $addEdit); ?>
	<form name="myForm" action="" method="post">
	<?= form_input("categoryID",$categoryID,array("type"=>"hidden")); ?>
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		...
		<tr class="contents">
			<td width="20%" class="label"><?= _LBL_HAZARD_CLASSIFICATION ?></td>
			<td width="80%"><div id="sp-class"><?= form_select("classID","",array(),"",array()); ?></div></td>
		</tr>
		<tr class="contents">
			<td class="label"><?= _LBL_HAZARD_CATEGORY ?></td>
			<td><?php
				$param1 = "categoryName";
				$param2 = isset($_POST[$param1])?$_POST[$param1]:$categoryName;
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
			<td align="center"><?php
				...
				echo "&nbsp;";
				$dOptions = array("type"=>"button","onClick"=>"redirectForm('hazCategoryList.php');");
