<?php
	//****************************************************************/
	// filename: newchemicalList_get.php
	// description: get list of the new chemical
	//***************************************************************/
	 ...
	 */
	$aColumns = array( ' ','c.Chemical_CAS','c.Chemical_Name',' ','c.RecordDate',' ' );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);
	array_push($aColumns2,'isNew','c.RecordBy');
	...
	*/
	...
	$sWhereCountAll .= " AND isNew = 1 AND isDeleted = 0 ";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				...
				elseif($key==($countArr-1)){
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'chemicalView.php?chemicalID='.$uniqueID.'&page=nc\')';
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//====================================================
					if($userLevelID<=2){
						$imgParam1 = $sys_config['images_path'].'icons/edit.gif';
						$imgParam2 = _LBL_EDIT;
						$imgParam3 = 'redirectForm(\'chemicalAddEdit.php?chemicalID='.$uniqueID.'&page=nc\')';
						$activity .= '&nbsp;'._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					}
					//====================================================
					...
					$row[] = !empty($aRow['RecordBy']) ? _get_User($aRow['RecordBy']) : '-';
				}
			}else{
				...
				if($col[$countCol-1]=='Active'){
				...
				}elseif($col[$countCol-1]=='RecordDate'){
					$dataCol = !empty($aRow[ $col[$countCol-1] ])?func_ymd2dmy($aRow[ $col[$countCol-1] ]):'-';
					$row[] = $dataCol;
					...
					$row[] = !empty($aRow[ $col[$countCol-1] ])?nl2br($aRow[ $col[$countCol-1] ]):'-';
				}
