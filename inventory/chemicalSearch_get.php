<?php
	//****************************************************************/
	// filename: chemicalSearch_get.php
	// description: get list of the chemical for adding the classified list
	//***************************************************************/
	...
	*/
	 ...
	 */
	$search = $_GET['search'];
	$sWhereCountAll = " WHERE 1 AND isDeleted = 0 AND isNew = 0 ";
	if($search!=''){
		$sWhereCountAll .= " AND (Chemical_Name LIKE '%".$search."%' OR Chemical_CAS LIKE '%".$search."%') ";
	}
...
//}
	...
	while ( $aRow = $db->sql_fetchrow($rResult) ){
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$aRow['Chemical_ID'].'" />';
				else{
					$chemName = func_fn_escape($aRow['Chemical_Name']);
					$onClick = "selectChemical('".$aRow['Chemical_ID']."','". $chemName ."','".$aRow['Chemical_CAS']."');";
					$row[] = "<a href=\"#\" onClick=\"".$onClick."\">". _LBL_CHOOSE ."</a>";
				}
			}else{
				...
				elseif($col[$countCol-1]=='Chemical_IUPAC')
					$row[] = (($aRow[ $col[$countCol-1] ]!='')?$aRow[ $col[$countCol-1] ]:$aRow['Chemical_Name']);
				else
