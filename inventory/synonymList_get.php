<?php
	//****************************************************************/
	// filename: synonymList_get.php
	// description: get list of the chemical synonym
	//***************************************************************/
	...
	$sIndexColumn = "cs.Synonym_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( ' ','c.Chemical_CAS','c.Chemical_Name','cs.Synonym_Name',' ' );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn,'c.Chemical_ID');

	/* DB table to use */
	$sTable = "tbl_Chemical_Synonyms cs";
	...
	* Joining
	*/
	$sJoin = ' RIGHT JOIN tbl_Chemical c ON cs.Chemical_ID = c.Chemical_ID ';
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND Active = 1 AND c.isActive = 1";
...
//}
	...
	while ( $aRow = $db->sql_fetchrow($rResult) ){
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" '. $disabled .' value="'.$aRow['Synonym_ID'].'" />';
				else{
					$activity = '';
					//====================================================
					$imgParam1 = $sys_config['images_path'].'icons/preview.gif';
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = 'redirectForm(\'synonymView.php?chemicalID='.$aRow['Chemical_ID'].'\')';
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//====================================================
					if($userLevelID<=2){
						$imgParam1 = $sys_config['images_path'].'icons/edit.gif';
						$imgParam2 = _LBL_EDIT;
						$imgParam3 = 'redirectForm(\'synonymAddEdit.php?chemicalID='.$aRow['Chemical_ID'].'\')';
						$activity .= '&nbsp;'._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					}
					//====================================================
