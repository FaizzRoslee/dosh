<?php
	//****************************************************************/
	// filename: HazardClassList_get.php [physical,health,environmental]
	...
	*/
	...
	$sWhereCountAll .= " AND (SELECT COUNT(*) AS total FROM tbl_Chemical_Classified WHERE Chemical_ID = tbl_Chemical.Chemical_ID) = 0 ";
	//$sWhereCountAll .= " AND isNew = 1 ";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				...
				elseif($key==($countArr-1)){
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'chemicalView.php?chemicalID='.$uniqueID.'&page=uc\')';
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//====================================================
					if($userLevelID<=2){
						$imgParam1 = $sys_config['images_path'].'icons/edit.gif';
						$imgParam2 = _LBL_EDIT;
						$imgParam3 = 'redirectForm(\'chemicalAddEdit.php?chemicalID='.$uniqueID.'&page=uc\')';
						$activity .= '&nbsp;'._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					}
					//====================================================
