<?php
	...
	func_header("",
				...
				$sys_config["includes_path"]."jquery/Uploader/prop-pdf-uploader.js,". // javascript
	...
	if(isset($_POST["save"])){
		...
		*/
		...
		// linuxhouse_20220524
        /** start isset: add-chemical-list-fields-task **/
        $propFlhPt      = isset($_POST["propFlhPt"])    ? $_POST["propFlhPt"]		: "";
        $propPhyHaz     = isset($_POST["propPhyHaz"])	? $_POST["propPhyHaz"]		: "";
        $propHlthHaz	= isset($_POST["propHlthHaz"])	? $_POST["propHlthHaz"]		: "";
        $propEnvHaz		= isset($_POST["propEnvHaz"])	? $_POST["propEnvHaz"]		: "";
        // $propPdf		= isset($_POST["fpath2"])	    ? $_POST["fpath2"]		    : "";
        $propPdf		= isset($_FILES["fpath2"])	    ? isset($_FILES["fpath2"])  : "";
        $old_propPdf	= isset($_POST["fpath2_old"])	? $_POST["fpath2_old"]		: "";
        
        // fpath2 info is not set
        if($propPdf && $_FILES["fpath2"]["error"] != 0){
            // assigh old file name from database to fpath2 during editting
            // old file name is stored in a hidden input field in the form
            $propPdf = $old_propPdf;
        }
        
        // fpath2 info is set
        if($propPdf && $_FILES["fpath2"]["error"] == 0){
            $fileName = $_FILES['fpath2']['name'];// gets file name
            $fileTmpName = $_FILES['fpath2']['tmp_name'];// get temporal location where file was saved
            $fileError = $_FILES['fpath2']['error'];// get file upload status error. 0=success, 1=error 
            
            $fileExt = explode('.', $fileName);// splits file name into an array with respect to '.'
            $fileActualExt = strtolower(end($fileExt));// end() gets the last element of an array
            
            // get file name without file extension
            $pathInfo = pathinfo($fileName);
            $fileActualName = $pathInfo['filename'];
            
            // checks if file was successfully uploaded temporarily
            if($fileError === 0)
            {
                // create unique id based on the current micro seconds and current time format
                $fileNameNew = $fileActualName . '-' . uniqid('', true) . '.' . $fileActualExt;
                
                // path where pdf file will be saved.
                // make sure folder permission is set to 'can view & modify content'
                // otherwise you will get errors or file value will be set to 1 bcos of isset($_FILES[])
                // returns true
                $fileDestination = $sys_config["upload_path"] . "/chemical-prop-pdf/" . $fileNameNew;
                // $fileDestination = $sys_config["upload_path"] . "/chemical/" . $fileNameNew;
                
                // upload the file by moving file from temporal location to server location
                if(move_uploaded_file($fileTmpName, $fileDestination))
                    $propPdf = $fileNameNew;// assign file name
            }
        }
        /** end isset: add-chemical-list-fields-task **/
		...
		if($chemicalID!=0){
			...
			$sqlUpd	.= " Chemical_Formula = ".quote_smart($chemFormula).","
					...
					// linuxhouse_20220524
                    /** start sql update: add-chemical-list-fields-task **/
					. " Prop_FlhPoint = ".quote_smart($propFlhPt).","
					. " Prop_PhyHazard = ".quote_smart($propPhyHaz).","
					. " Prop_HlthHazard = ".quote_smart($propHlthHaz).","
					. " Prop_EnvHazard = ".quote_smart($propEnvHaz).","
                    . " Prop_PDF = ".quote_smart($propPdf).","
					/** end sql update: add-chemical-list-fields-task **/
			...
			$sqlIns	.=  " Chemical_Formula,Chemical_Mol,Chemical_Structure,"
					...
					/** start sql add table columns: add-chemical-list-fields-task **/
					. " Prop_FlhPoint,Prop_PhyHazard,Prop_HlthHazard,Prop_EnvHazard,"
					. " Prop_PDF,"
					/** start sql add table columns: add-chemical-list-fields-task **/
					/*********/
					. " isCWC,HS_Code,";
			...
			$sqlIns	.= " ".quote_smart($chemFormula).",".quote_smart($chemMol).",".quote_smart($chemStructure).","
					...
					// linuxhouse_20220524
					/** start sql add table values: add-chemical-list-fields-task **/
					. " ".quote_smart($propFlhPt).", ".quote_smart($propPhyHaz).", ".quote_smart($propHlthHaz).", ".quote_smart($propEnvHaz).","
					. " ".quote_smart($propPdf).", "
					/** end sql add table values: add-chemical-list-fields-task **/
...
?>
    ...
    <!-- linuxhouse_20220524 -->
    <!-- start form: add-chemical-list-fields-task -->
    <!--add  enctype="multipart/form-data" to form-->
	<form name="myForm" action="" method="post" enctype="multipart/form-data">
	<!-- end form: add-chemical-list-fields-task -->
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		...
		<tr class="contents">
			...
			<td colspan="3"><?php
				$param1 = "propFlhPt";
				$param2 = isset($_POST[$param1])?$_POST[$param1]:$propFlhPt;
				$param3 = array("rows"=>"2","cols"=>"60");
				echo form_textarea($param1,$param2,$param3);
			?></td>
		</tr>
		<tr class="contents">
			...
			<td colspan="3"><?php
				$param1 = "propPhyHaz";
				$param2 = isset($_POST[$param1])?$_POST[$param1]:$propPhyHaz;
				$param3 = array("rows"=>"2","cols"=>"60");
				echo form_textarea($param1,$param2,$param3);
			?></td>
		</tr>
		<tr class="contents">
			...
			<td colspan="3"><?php
				$param1 = "propHlthHaz";
				$param2 = isset($_POST[$param1])?$_POST[$param1]:$propHlthHaz;
				$param3 = array("rows"=>"2","cols"=>"60");
				echo form_textarea($param1,$param2,$param3);
			?></td>
		</tr>
		<tr class="contents">
			...
			<td colspan="3"><?php
				$param1 = "propEnvHaz";
				$param2 = isset($_POST[$param1])?$_POST[$param1]:$propEnvHaz;
				$param3 = array("rows"=>"2","cols"=>"60");
				echo form_textarea($param1,$param2,$param3);
			?></td>
		</tr>
        <tr class="contents">
			<td class="label"><?= _LBL_UPLOAD_PDF_FILE ?></td>
			<td colspan="3">
                <?php
                    $param1 = 'fpath2';
                    $param2 = isset($_POST[$param1])?$_POST[$param1]:'';
                    ...
                    echo form_input($param1,$param2,$param3);
                    echo form_input('fpath2_old',$propPdf,array("type"=>"hidden"));
                ?>
            </td>
		</tr>
	...
	// linuxhouse_20220524
	// start javascript: add-chemical-list-fields-task
    document.getElementById('fpath2').addEventListener('change', checkFile, false);
    // end javascript: add-chemical-list-fields-task
