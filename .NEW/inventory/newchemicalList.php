<?php
	//****************************************************************/
	// filename: newchemicalList.php
	// description: list of the new chemical
	//***************************************************************/
	...
	if(isset($_POST["checkbox"]) && isset($_POST["delete"])){
		...
		foreach($_POST["checkbox"] as $value){
			...
			/*********/
			func_add_audittrail($updBy,$msg." [".$value."]","Inventory-New Chemical","tbl_Chemical".$value);
		}
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $('#example').dataTable( {
			...
			"sPaginationType": "full_numbers",
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "newchemicalList_get.php?userLevelID=<?= $userLevelID ?>",
			"aoColumns": [
	...
	<?php func_window_open2(_LBL_NEW_CHEMICAL); ?>
		...
		<div id="container">
			<div class="demo_jui">
				<table style="width:100%">
					<tr>
						<td width="60%">&nbsp;</td>
						<td width="40%" align="right"><?= form_button("delete",_LBL_DELETE,array("type"=>"submit","onClick"=>"return confirm_msgbox();")); ?></td>
					</tr>
				</table>
				<?php 	
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							...
							<th width="36%"><?= _LBL_CHEMICAL_NAME ?></th>
							<th width="25%"><?= _LBL_RECORD_BY ?></th>
							<th width="15%"><?= _LBL_RECORD_DATE ?></th>
					...
					<tfoot>
						<tr>
							...
							<th><input type="hidden" name="search_recordby" value="<?= _LBL_SEARCH .' '. _LBL_RECORD_BY ?>" class="search_init" /></th>
							<th><input type="hidden" name="search_date" value="<?= _LBL_SEARCH .' '. _LBL_RECORD_DATE ?>" class="search_init" /></th>
