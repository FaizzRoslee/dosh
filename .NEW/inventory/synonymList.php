<?php
	//****************************************************************/
	// filename: synonymList.php
	// description: list of the chemical synonyms
	//***************************************************************/
	...
	/*********/
	if(isset($_POST["checkbox"]) && isset($_POST["inactive"])){
		$active = 0;
		/*********/
		...
		foreach($_POST["checkbox"] as $value){
			$sqlDel = "UPDATE tbl_Chemical_Synonyms SET"
					. " Active = ".quote_smart($active).","
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Synonym_ID = ".quote_smart($value)." LIMIT 1"; //set active==0 instead delete from table;
			...
			/*********/
			func_add_audittrail($updBy,$msg." [".$value."]","Admin-Chemical Synonyms","tbl_Chemical_Synonyms".$value);
		}
...
?>
	...
	<?php func_window_open2(_LBL_SYNONYM); ?>
				...
				<table style="width:100%">
					<tr>
						...
						<td width="40%" align="right"><?php
							echo form_button("add",_LBL_ADD,array("type"=>"button","onClick"=>"redirectForm('synonymAddEdit.php');"));
							/*********/
							echo "&nbsp;";
							/*********/
							$dOthers = array("type"=>"submit","onClick"=>"return confirm_msgbox();");
							echo form_button("inactive",_LBL_DELETE,$dOthers);
							/*********/
						?></td>
					</tr>
				</table>
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							...
							<th width="36%"><?= _LBL_SYNONYM_NAME ?></th>
					...
					<tfoot>
						<tr>
							...
							<th><input type="text" name="search_synonym" value="<?= _LBL_SEARCH .' '. _LBL_SYNONYM_NAME ?>" class="search_init" /></th>
