<?php
	 ...
	 */
	$aColumns = array( ' ','Chemical_CAS','Chemical_Name','Chemical_IUPAC',' ','IF(isCWC=1, "'. _LBL_YES .'","'. _LBL_NO .'")',' ' );
	$aColumns2 = $aColumns;
	...
	*/
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND isDeleted = 0 AND isActive = ".$_GET['status']." AND isNew = 0 ";
		...
		foreach($aColumns as $key => $detail){
			...
			if($detail==' '){
				...
				elseif($key==$maxArr){
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'chemicalView.php?chemicalID='.$uniqueID.'\')';
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//====================================================
					if($userLevelID<=2){
						$imgParam1 = $sys_config['images_path'].'icons/edit.gif';
						$imgParam2 = _LBL_EDIT;
						$imgParam3 = 'redirectForm(\'chemicalAddEdit.php?chemicalID='.$uniqueID.'\')';
						$activity .= '&nbsp;'._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					}
					//====================================================
