<?php
	...
	/*********/
	if(isset($_POST["delete"]) && isset($_POST["checkbox"])){ //die;
		/*********/
		$updBy = $Usr_ID;
		...
		foreach($_POST["checkbox"] as $value){
			...
			if($resDel){ 
				...
				$sqlIns = "INSERT INTO tbl_Submission_Record ("
						...
						. " ) VALUE ("
						. " ". quote_smart($msg) .",". quote_smart("Delete Submission") .",". quote_smart($updBy) .",NOW(),61,". quote_smart($value)
						. " )";
	...
	// $dSubmit = "COALESCE((SELECT Sub_Record_Date FROM tbl_Submission_Record WHERE Submission_ID = s.Submission_ID AND Sub_Record_Status = 11 ORDER BY Sub_Record_Date DESC LIMIT 1),'asas')";
?>
	...
	jQuery(document).ready(function() {
		...
		var oTable = $("#example").dataTable( {
			...
			"bJQueryUI": true,
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "submissionList_get.php?type=<?= $colTypeName ?>&levelID=<?= $levelID ?>&Usr_ID=<?= $Usr_ID ?>&langdesc=<?= $langdesc ?>",
			"aoColumns": [
		...
		$("input[name=checkAll]").live("click",function(){
			$(".cls-chck").prop("checked", $(this).is(":checked") );
		});
	} );
	</script>
	<?php func_window_open2(_LBL_SUBMISSION ." :: ". _LBL_LIST); ?>
		...
		<div id="container">
			<div class="demo_jui">
					...
					<tr>
						...
						<td width="90%" align="right"><?php
							if($levelID>=6 || $levelID<=2){
								echo form_button("add",_LBL_ADD . " " . _LBL_SUBMISSION,array("type"=>"button","onClick"=>"redirectForm('submissionAddEdit.php')"));
								echo "&nbsp;";
								echo form_button("upload",_LBL_UPLOAD_BULK_SUBMISSION,array("type"=>"button","onClick"=>"redirectForm('submissionUpload.php')"));
								echo "&nbsp;";
								echo form_button("delete",_LBL_DELETE,array("type"=>"submit","onClick"=>"return confirm_msgbox();"));
							}
						?></td>
					</tr>
				</table>
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							<th class="center">&nbsp;<input type="checkbox" name="checkAll" id="checkAll" <?= (($levelID>=6||$levelID==1)?"":"disabled"); ?> /></th>
