	...
	if(isset($_POST["fn"])){
			...
			if($quantity>0){
				...
				for($i=1;$i<=$quantity;$i++){
					...
					$str .= '<br />'
						...
						. 	'<tr class="contents">'
						.		'<td class="label title" colspan="4"><input type="checkbox" name="chkSelect_'.$i.'" id="chkSelect_'.$i.'" value="1" checked class="cl_chkbx" />'
						. 			_LBL_SUBSTANCE .' '. $i .'</td>'
						.	'</tr>'
						.	'<tr class="contents">'
		...
		elseif($_POST["fn"]=="mixturechemical"){
			...
			for(;$i<=$mixture;$i++){
				...
				$dName = "source_".$row."_".$i;
				$dOptions = $arrSource;
				$str .= form_select($dName,"",$dOptions,"",array("class"=>"slSource")) ;
