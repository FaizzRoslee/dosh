<?php	
	...
	$aColumns = array( ' ','s.Submission_Submit_ID','uc.Company_Name','sl.Level_Name',$dCount,$dSubmit,' ','ss.Status_Desc','s.CheckedBy',' ');
	$i_nochem = 4;
	$indexDate = 5;
	$i_datesubmit = 5;
	$i_dateexp = 6;
	...
	*/
	...
	$sWhereCountAll = " WHERE 1 AND s.isDeleted = 0 AND s.Status_ID IN (41,42,43,52) ";
	if($levelID==5){ $sWhereCountAll .= " AND uc.State_Reg = ". quote_smart(_get_StrFromCondition('sys_User_Staff','State_ID','Usr_ID',$Usr_ID)); }
		...
		for ( $i=0 ; $i<$max_column ; $i++ ) {
			...
			else
			{
				...
				elseif($i == ($max_column-1) ) { //last
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'submissionView_state.php?submissionID='.$uniqueID.'&new=15\')';
