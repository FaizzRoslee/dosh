<?php
	//****************************************************************/
	// filename: userList_get.php
	// description: get list of the user
	//***************************************************************/
	 ...
	 */
	$aColumns = array( " ","uc.Reg_ID","uc.Company_No","uc.Company_Name","s.State_Name","u.Usr_LoginID","u.Active"," " );
	$aColumns2 = $aColumns;
	...
	*/
	//$sJoin = '';
	$sJoin = " LEFT OUTER JOIN sys_User_Client uc ON u.Usr_ID = uc.Usr_ID "
			. " LEFT OUTER JOIN sys_Level l ON u.Level_ID = l.Level_ID "
			. " LEFT OUTER JOIN sys_State s ON uc.State_Reg = s.State_ID ";
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND l.Level_Type = 2 AND u.Level_ID = ". quote_smart($_GET["levelID"]) ." AND u.isDeleted = 0 ";
	if($_GET["status"]!="")	
		$sWhereCountAll .= " AND u.Active = ". quote_smart($_GET["status"]);
		...
		foreach($aColumns as $key => $detail){
			...
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = "redirectForm('userView.php?Usr_ID=".$uniqueID."&levelID=".$_GET["levelID"]."')";
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					/*********/
					$imgParam1 = $sys_config["images_path"]."icons/edit.gif";
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = "redirectForm('userAddEdit.php?Usr_ID=".$uniqueID."&levelID=".$_GET["levelID"]."')";
					$activity .= "&nbsp;"._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					/*********/
					$url_pdf = $sys_config["report_path"]."regCert_pdf.php?Usr_ID=".$uniqueID;
					$imgParam1 = _LBL_CERTIFICATION;
					$imgParam2 = "funcPopup('$url_pdf',1000,600)";
					$activity .= "&nbsp;"._get_imagedoc($imgParam1,$imgParam2);
					/*********/
					$row[] = "<nobr>".$activity."</nobr>";
				}
			}else{
