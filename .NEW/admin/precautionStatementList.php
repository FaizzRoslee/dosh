<?php
	//****************************************************************/
	// filename: precautionStatementList.php
	// description: list of the precaution statement
	//***************************************************************/
	...
	if(isset($_POST['checkbox']) && (isset($_POST['active'])||isset($_POST['inactive']))){
		...
		foreach($_POST['checkbox'] as $value){
			$sqlDel = "UPDATE tbl_Precaution_Statement SET"
					. " Active = ".quote_smart($active).","
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Statement_ID = ".quote_smart($value)." LIMIT 1";
			$resDel = $db->sql_query($sqlDel,END_TRANSACTION) or die(print_r($db->sql_error()));
			//=================================================
			$msg = ($resDel) ? 'Update Active/Inactive Successfully' : 'Update Active/Inactive Unsuccessfully';
			//=================================================
			func_add_audittrail($updBy,$msg.' ['.$value.']','Admin-Precaution Statement','tbl_Precaution_Statement'.$value);
		}
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $('#example').dataTable( {
			...
			"sPaginationType": "full_numbers",
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "precautionStatementList_get.php",
			"aoColumns": [
				{ "sName": " ", "bSortable": false},
				{ "sName": "pt.Precaution_Name" },
				{ "sName": "ps.Statement_Code" },
				{ "sName": "ps.Statement_Desc" },
				{ "sName": "ps.Active" },
				{ "sName": " ", "bSortable": false }
 			],
			"aaSorting": [[ 2, 'asc' ]],
	...
	<?php func_window_open2(_LBL_PRECAUTIONARY_STATEMENT); ?>
				...
				<table style="width:100%">
					<tr>
						...
						<td width="40%" align="right"><?php
							...
							$param1 = 'add';
							$param2 = _LBL_ADD;
							$param3 = array('type'=>'button','onClick'=>'redirectForm(\'precautionStatementAddEdit.php\')');
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							...
							<th width="19%"><?= _LBL_PRECAUTION_TYPE ?></th>
							<th width="10%"><?= _LBL_PRECAUTION_CODE ?></th>
							<th width="42%"><?= _LBL_PRECAUTION_DESC ?></th>
							<th width="15%"><?= _LBL_STATUS ?></th>
					...
					<tfoot>
						<tr>
							...
							<th><input type="text" name="search_type" value="<?= _LBL_SEARCH .' '. _LBL_PRECAUTION_TYPE ?>" class="search_init" /></th>
							<th><input type="text" name="search_code" value="<?= _LBL_SEARCH .' '. _LBL_PRECAUTION_CODE ?>" class="search_init" /></th>
							<th><input type="text" name="search_name" value="<?= _LBL_SEARCH .' '. _LBL_PRECAUTION_DESC ?>" class="search_init" /></th>
