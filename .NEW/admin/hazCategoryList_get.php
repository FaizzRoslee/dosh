<?php
	//****************************************************************/
	// filename: hazCategoryList_get.php
	// description: get list of the hazard classification [physical,health,environmental]
	//***************************************************************/
	 ...
	 */
	$aColumns = array( " ","ht.Type_Name","hc.Class_Name","hcat.Category_Name","hs.Signal_Name","hcat.Active"," " );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);

	/* DB table to use */
	$sTable = "tbl_Hazard_Category hcat";
	...
	* Joining
	*/
	$sJoin = " LEFT OUTER JOIN tbl_Hazard_Type ht ON hcat.Type_ID = ht.Type_ID";
	$sJoin .= " LEFT OUTER JOIN tbl_Hazard_Classification hc ON hcat.Class_ID = hc.Class_ID";
	$sJoin .= " LEFT OUTER JOIN tbl_Hazard_Signal hs ON hcat.Signal_ID = hs.Signal_ID";
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND hcat.isDeleted = 0 ";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = "redirectForm('hazCategoryView.php?categoryID=$uniqueID')";
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					/*********/
					$imgParam1 = $sys_config["images_path"]."icons/edit.gif";
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = "redirectForm('hazCategoryAddEdit.php?categoryID=$uniqueID')";
					$activity .= "&nbsp;"._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					/*********/
