<?php
	...
	$tbl .= '
			<td width="'. $width_12_15 .'" style="'. $text_align_center .'">'. $trade_chemical .'</td>
			<td width="7%" style="'. $text_align_center .'">'. _LBL_CAS_NO .'</td>
			<td width="15%" style="'. $text_align_center .'"><nobr>'. _LBL_HAZARD_CLASSIFICATION .'</nobr></td>
			<td width="7%" style="'. $text_align_center .'">'. _LBL_APPROVED_DATE .'</td>
			<td width="8%" style="'. $text_align_center .'">'. _LBL_EXPIRED_DATE .'</td>
		';
			$minus_sup = 2;
			if($uLevelID!=7){
				$minus_sup = 0;
	...
	$tbl .= '
		...
		';
			...
			//=========================================	
			$wid_all = ($uLevelID!=6 && $uLevelID!=7) ? '100%' :(($chemType==2)?'82%':'79%' );
...
*/			foreach($arrayData1 as $key => $detail1){
	...
	$tbl .= '
			<td width="'. $width_12_15 .'" '. $td_mid .'>'. (isset($arrayChem[$i]['Chemical_ID'])?nl2br(func_fn_escape(_get_StrFromCondition('tbl_Chemical','Chemical_Name','Chemical_ID',$arrayChem[$i]['Chemical_ID']))):'&nbsp;') .'</td>
			<td width="7%" '. $td_mid_center .'>'. (isset($arrayChem[$i]['Chemical_ID'])?_get_StrFromCondition('tbl_Chemical','Chemical_CAS','Chemical_ID',$arrayChem[$i]['Chemical_ID']):'&nbsp;') .'</td>
		';
					if($i==0){
						//=========================================
	...
	$tbl .= '
			...
			<td width="7%" '. $td_rowspan . $td_mid_center .'>'. (_getSubmitDate($detail1['Submission_ID'])) .'</td>
	...
	$tbl .= '
		...
		';
						...
						if($uLevelID!=6){
	$tbl .= '			
			<td width="'. (($uLevelID==8)?$width_10_13:'8%') .'" '. $td_mid_right .'>'. _LBL_TOTAL .'</td>
