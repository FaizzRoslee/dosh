...
?>
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
			<td width="3%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_BIL ?></td>
			<?php 
					$minus = 1;
					if($chemType==2){
			?>
			<td width="19%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_PRODUCT_NAME ?></td>
			<?php 
						$minus = 0;
			...
			?>
			<td width="10%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= $trade_chemical ?></td>
			<td width="7%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_CAS_NO ?></td>
			<td width="18%" class="label" style="<?= $text_align_center ?>" colspan="3"><nobr><?= _LBL_HAZARD_CLASSIFICATION ?></nobr></td>
			<td width="7%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_APPROVED_DATE ?></td>
			<?php 
					$minus_sup = 2;
					if($lvlType!=7 && $uLevelID!=5){
						$minus_sup = 0;
			?>
			<td width="8%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_TOTAL_QUANTITY_SUPPLY .'<br />('. _LBL_TONNE_YEAR .')' ?></td>
			<td width="10%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_LIST_SUPPLIED_ENTITY ?></td>
			<?php 
					} 
					$minus_use = 2;
					if($lvlType!=6 && $uLevelID!=5){
						$minus_use = 0;
			?>
			<td width="8%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_TOTAL_QUANTITY_USE .'<br />('. _LBL_TONNE_YEAR .')' ?></td>
			<td width="10%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_LIST_SUPPLIER_ENTITY ?></td>
			<?php 
					} 
			?>
		</tr>
		<tr class="contents">
			<td width="6%" class="label" style="<?= $text_align_center ?>"><nobr><?= _LBL_PHY ?></nobr></td>
			<td width="6%" class="label" style="<?= $text_align_center ?>"><nobr><?= _LBL_HEALTH ?></nobr></td>
			<td width="6%" class="label" style="<?= $text_align_center ?>"><nobr><?= _LBL_ENV ?></nobr></td>
		</tr>
		<?php
		...
		?>
		<tr class="contents">
			<td colspan="<?= (12-$minus-$minus_sup-$minus_use) ?>" class="label"><?= ':: ' . (($lvlName!='')?$lvlName:_LBL_COMPANY_NAME) .' - '. _get_StrFromCondition('sys_User_Client','Company_Name','Usr_ID',$uID); ?></td>
		</tr>
		<?php
								}
		?>
		<tr class="contents">
			...
			<td <?= $td_rowspan.$td_mid; ?>><?php
				...
				if( is_array($ex_phCategory) && count($ex_phCategory)>0 ){
			...
			<td <?= $td_rowspan.$td_mid; ?>><?php
				...
				if( is_array($ex_hhCategory) && count($ex_phCategory)>0 ){
		...
		?>
		<tr class="contents">
			<td class="label" colspan="<?= (7-$minus) ?>" <?= $td_mid_right; ?>>&nbsp;</td>
			<?php
		...
		<tr class="contents" height="30">
			<td colspan="<?= (12-$minus-$minus_sup-$minus_use) ?>" <?= $td_mid_center; ?>><?= _LBL_NO_RECORD ?></td>
		</tr>
		<?php
					}
					//================================================state looping================================================\\
