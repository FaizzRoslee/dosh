<?php
	...
	//	$chemType=1;
			...
			if($chemType==2){
	$tbl .= '
			<td width="11%" class="label" style="'. $text_align_center .'" rowspan="2">'. _LBL_PRODUCT_NAME .'</td>
		';
				$minus = 0;
				$width1 = 30;
				$width2 = 0;
	...
	$tbl .= '
			<td width="'. (11+$width2) .'%" style="'. $text_align_center .'" rowspan="2">'. $trade_chemical .'</td>
			<td width="9%" style="'. $text_align_center .'" rowspan="2">'. _LBL_CAS_NO .'</td>
			<td width="'.$width1.'%" style="'. $text_align_center .'" colspan="3"><nobr>'. _LBL_HAZARD_CLASSIFICATION .'</nobr></td>
			<td width="9%" style="'. $text_align_center .'" rowspan="2">'. _LBL_APPROVED_DATE .'</td>
			<td width="9%" style="'. $text_align_center .'" rowspan="2">'. _LBL_EXPIRED_DATE .'</td>
		';
			if($uLevelID!=7){
	$tbl .= '
	...
	$tbl .= '
		...
		';
					...
					if($i==0&&$chemType==2){
	$tbl .= '
			<td width="11%" '. $td_rowspan . $td_mid .'>'. $detail1['Detail_ProductName'] .'</td>
		';
					}
	$tbl .= '
			<td width="'. (11+$width2) .'%" '. $td_mid .'>'. (isset($arrayChem[$i]['Chemical_ID'])?nl2br(func_fn_escape(_get_StrFromCondition('tbl_Chemical','Chemical_Name','Chemical_ID',$arrayChem[$i]['Chemical_ID']))):'&nbsp;') .'</td>
			<td width="9%" '. $td_mid_center .'>'. (isset($arrayChem[$i]['Chemical_ID'])?_get_StrFromCondition('tbl_Chemical','Chemical_CAS','Chemical_ID',$arrayChem[$i]['Chemical_ID']):'&nbsp;') .'</td>
		';
					if($i==0){
						//=========================================
	...
	$tbl .= '
			...
			<td width="9%" '. $td_rowspan . $td_mid_center .'>'. (_getSubmitDate($detail1['Submission_ID'])) .'</td>
			<td width="9%" '. $td_rowspan . $td_mid_center .'>'. (_getExpiredDate($detail1['Submission_ID'])) .'</td>
		';
						//=========================================
	...
	$tbl .= '
		<tr style="'.$label4.'">
			<td width="73%" colspan="'. (9-$minus) .'" '. $td_mid_right .'>&nbsp;</td>
		';
						if($uLevelID!=7){
	$tbl .= '
			<td width="9%" '. $td_mid_right .'>'. _LBL_TOTAL .'</td>
	...
	$tbl .= '
		<tr height="30">
			<td width="'. $wid_all .'" colspan="'. (13-$minus) .'" '. $td_mid_center .'>'. _LBL_NO_RECORD .'</td>
		</tr>
		';
			}
	...
	//	$chemType=1;
			...
			if($chemType==2){
	$tbl .= '
			<td width="10%" class="label" style="'. $text_align_center .'" rowspan="2">'. _LBL_PRODUCT_NAME .'</td>
		';
				$minus = 0;
				$width1 = 24;
				$width2 = 0;
	...
	$tbl .= '
			<td width="'. (9+$width2) .'%" style="'. $text_align_center .'" rowspan="2">'. $trade_chemical .'</td>
			<td width="8%" style="'. $text_align_center .'" rowspan="2">'. _LBL_CAS_NO .'</td>
			<td width="'.$width1.'%" style="'. $text_align_center .'" colspan="3"><nobr>'. _LBL_HAZARD_CLASSIFICATION .'</nobr></td>
			<td width="8%" style="'. $text_align_center .'" rowspan="2">'. _LBL_APPROVED_DATE .'</td>
			<td width="8%" style="'. $text_align_center .'" rowspan="2">'. _LBL_EXPIRED_DATE .'</td>
			<td width="6%" style="'. $text_align_center .'" rowspan="2">'. _LBL_TOTAL_QUANTITY_SUPPLY .'<br />('. _LBL_TONNE_YEAR .')</td>
			<td width="9%" style="'. $text_align_center .'" rowspan="2">'. _LBL_LIST_SUPPLIED_ENTITY .'</td>
			<td width="6%" style="'. $text_align_center .'" rowspan="2">'. _LBL_TOTAL_QUANTITY_SUPPLY .'<br />('. _LBL_TONNE_YEAR .')</td>
			<td width="9%" style="'. $text_align_center .'" rowspan="2">'. _LBL_LIST_SUPPLIER_ENTITY .'</td>
		</tr>
		<tr style="'.$label.'">
			<td width="'. ($width1/3) .'%" style="'. $text_align_center .'"><nobr>'. _LBL_PHY .'</nobr></td>
			<td width="'. ($width1/3) .'%" style="'. $text_align_center .'"><nobr>'. _LBL_HEALTH .'</nobr></td>
	...
	$tbl .= '
		...
		';
					...
					if($i==0&&$chemType==2){
	$tbl .= '
			<td width="10%" '. $td_rowspan . $td_mid .'>'. $detail1['Detail_ProductName'] .'</td>
		';
					}
	$tbl .= '
			<td width="'. (9+$width2) .'%" '. $td_mid .'>'. (isset($arrayChem[$i]['Chemical_ID'])?nl2br(func_fn_escape(_get_StrFromCondition('tbl_Chemical','Chemical_Name','Chemical_ID',$arrayChem[$i]['Chemical_ID']))):'&nbsp;') .'</td>
	...
	$tbl .= '
			...
			<td width="8%" '. $td_rowspan . $td_mid_center .'>'. (_getSubmitDate($detail1['Submission_ID'])) .'</td>
			<td width="8%" '. $td_rowspan . $td_mid_center .'>'. (_getExpiredDate($detail1['Submission_ID'])) .'</td>
		';
						//=========================================
						$totalSupply = $detail1['Detail_TotalSupply'];
						$sum_totalSupply += $totalSupply;
						...
						if($uLevelID!=7 && $uLevelID!=5){ 
	$tbl .= '
			<td width="6%" '. $td_rowspan . $td_mid_right .'>'. (number_format($totalSupply,1)) .'</td>
			<td width="9%" '.  $td_rowspan . $td_mid .'>
		';
				foreach($ex_entityTo as $coID){
					if($coID!='')
						$tbl .= _get_StrFromCondition('tbl_Client_Company','Company_Name','Company_ID',$coID).' ('. ((_get_StrFromCondition('tbl_Client_Company','Active','Company_ID',$coID)==1)?_LBL_ACTIVE:_LBL_INACTIVE) .');<br />';
	...
	$tbl .= '
		...
		';
						...
						if($uLevelID!=6 && $uLevelID!=5){
	$tbl .= '
			<td width="6%" '. $td_rowspan . $td_mid_right .'>'. (number_format($totalUse,1)) .'</td>
			<td width="9%" '. $td_rowspan . $td_mid .'>
		';
				foreach($ex_entityFrom as $coID){
					if($coID!='')
	...
	$tbl .= '
		<tr style="'.$label4.'">
			<td width="62%" colspan="'. (8-$minus) .'" '. $td_mid_right .'>&nbsp;</td>
			<td width="8%" '. $td_mid_right .'>'. _LBL_TOTAL .'</td>
			<td width="6%" '. $td_mid_right .'>'. (number_format($sum_totalSupply,1)) .'</td>
			<td width="9%" '. $td_mid_right .'>'. _LBL_TOTAL .'</td>
			<td width="6%" '. $td_mid_right .'>'. (number_format($sum_totalUse,1)) .'</td>
			<td width="9%">&nbsp;</td>
		</tr>
		';
				}
