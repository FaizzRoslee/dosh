...
?>
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
			...
			<td width="7%" class="label" style="<?= $text_align_center ?>" rowspan="2"><?= _LBL_EXPIRED_DATE ?></td>
			<?php 
					$minus_sup = 2;
		...
		<tr class="contents">
			...
			<td <?= $td_rowspan.$td_mid_center; ?>><?= _getExpiredDate($detail1['Submission_ID']); ?></td>
			<?php
						//=========================================					
		...
		?>
		<tr class="contents">
			<td class="label" colspan="<?= (8-$minus) ?>" <?= $td_mid_right; ?>>&nbsp;</td>
			<?php
						if($uLevelID!=7){
			?>
		...
		<tr class="contents" height="30">
			<td colspan="<?= (11-$minus-$minus_sup-$minus_use) ?>" <?= $td_mid_center; ?>><?= _LBL_NO_RECORD ?></td>
		</tr>
		<?php
			}
		?>
	...
	<table width="100%">
		<tr>
			<td align="center"><?php
				if($bil>0){
					$url_pdf = 'report01_to_pdf_supp.php?rptType='.$rptType.'&chemType='.$chemType.'&rptYear='.$rptYear;
