<?php	
	...
	// $aColumns = array( ' ','Feedback_Name','Feedback_Contact','Feedback_Email','Type_Name_'.$lang,'Feedback_Subject',"DATE_FORMAT(RecordDate,'%d-%m-%Y')",' ');
	$aColumns = array( ' ',' ','Feedback_Name','Type_Name_'.$lang,'Feedback_Subject',"IF(Status = 2, '". _LBL_REPLIED ."', '". _LBL_NEW ."')","DATE_FORMAT(RecordDate,'%d-%m-%Y')",' ');
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "Feedback_ID";
	...
	* Joining
	*/
	$sJoin = " LEFT JOIN tbl_Feedback_Type ON tbl_Feedback.Type_ID = tbl_Feedback_Type.Type_ID ";
		...
		for ( $i=0 ; $i<$max_column ; $i++ ) {
			...
			else
			{
				if($i == 0) {
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'feedback-view.php?id='.$uniqueID.'\')';
					$activity .= _get_imagebutton2($imgParam1,$imgParam2,$imgParam3);
					//====================================================
					$imgParam1 = $sys_config['images_path'].'icons/check.gif';
					$imgParam2 = _LBL_REPLY;
					$imgParam3 = 'redirectForm(\'reply.php?id='.$uniqueID.'\')';
