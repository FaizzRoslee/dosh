<?php
	...
	function sendmail($email_content=array()){
		...
		try {
			...
			if(isset($email_content["emailCc"]) && !empty($email_content["emailCc"]) && is_array($email_content["emailCc"])){
				foreach($email_content["emailCc"] as $email){
					if(isset($email[0]) && !empty($email[0]))
						$mail->AddCC($email[0], $email[1]);
				}
			}
	...
	function func_email4($strFrom="",$strTo="",$strSubject="",$strMsg="",$strCC=""){
		$arr = array(
				"emailFrom" => $strFrom,
				"emailTo" => array(array($strTo,"")),
				"emailCc" => $strCC,
