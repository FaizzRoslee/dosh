<?php	
	...
	if(in_array($levelID,array(6,7,8))){
		...
		$statusColumn = "IF(s.Status_ID = '11' OR s.Status_ID = '21', '". _LBL_PENDING_ACKNOWLEDGEMENT ."', ss.Status_Desc)";
	}
	else $statusColumn = "ss.Status_Desc";
	
	if($levelID<5){
		$aColumns = array( " ","s.Submission_Submit_ID","uc.Company_Name","sl.Level_Name",$colTypeName,$dSubmit,"s.BulkSubmission",$statusColumn ." AS Status_Desc","s.CheckedBy"," ");
		$indexDate 		= 5;
		$i_datesubmit 	= 5;
		$indexStatus 	= 7;
		$i_status 		= 8;
			...
			else
			{
				if($i == 0) {
					if($statusID==61||$statusID==62)
						...
						$chbx = '<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" '. (($levelID==1)?'class="cls-chck"':'disabled') .' />';
