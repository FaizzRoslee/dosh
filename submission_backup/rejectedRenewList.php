<?php
	//****************************************************************/
	// filename: rejectedRenewList.php
	// description: list of the rennew submission that has been rejected 
	//***************************************************************/
...
/*
	...
	if($levelID==5){ $dWhere = array("AND"=>"=0"); }
		...
		$dWhere1 = array("AND s.Status_ID"=>"IN (4)");
		$dWhere2 = array();
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $('#example').dataTable( {
			...
			"bJQueryUI": true,
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "rejectedRenewList_get.php?type=<?= $colTypeName ?>&levelID=<?= $levelID ?>&Usr_ID=<?= $Usr_ID ?>",
			"aoColumns": [
	...
	<?php func_window_open2(_LBL_RENEW_SUBMISSION .' :: '. _LBL_REJECTED); ?>
					...
					/*
						foreach($arrayData as $detail){
							...
							$reason = _get_StrFromManyConditions('tbl_Submission_Record','Sub_Record_Reason',
											array('AND Sub_Record_Status'=>'= 4','AND Submission_ID'=>'= '.quote_smart($uniqueID),'ORDER BY'=>'Sub_Record_ID DESC')
									);
							...
							$data = '-';
							if($reason!='') $data = nl2br($reason);
							if($contact!='') $data .= '<br />[<strong>'. _LBL_CONTACT_NO .' : </strong>'. $contact .']';
							//====================================================
