<?php	
	...
	if($_GET["levelID"]<5){
		$aColumns = array( " ","s.Submission_Submit_ID","uc.Company_Name","sl.Level_Name".$langdesc,$colTypeName,$dSubmit,$dReason,"ss.Status_Desc".$langdesc,$dContact,"s.CheckedBy"," ");
		$indexDate = 5;
		$i_datesubmit = 5;
		$i_reason = 6;
		$i_contact = 8;
		...
		$aColumns = array( " ","s.Submission_Submit_ID",$colTypeName,$dSubmit,$dReason,"ss.Status_Desc".$langdesc,$dContact,"s.CheckedBy"," ");
		$indexDate = 3;
		$i_datesubmit = 3;
		$i_reason = 4;
		$i_contact = 6;
	...
	*/
	...
	if($levelID==5){ $sWhereCountAll .= " AND = 0 "; }
		...
		$sWhereCountAll .= " AND s.isDeleted = 0 AND s.Status_ID IN (2) ";
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if($aColumns[$i]!=" "){
				...
				elseif($iColumnIndex==$i_reason){
					$sWhere .= $aColumns[$i_reason]." LIKE '%".mysqli_real_escape_string($db->db_connect_id, $_GET['sSearch'])."%' OR ";
					$sWhere .= $aColumns[$i_contact]." LIKE '%".mysqli_real_escape_string($db->db_connect_id, $_GET['sSearch'])."%' ";
				}
				else
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( $_GET["bSearchable_".$i] == "true" && $_GET["sSearch_".$i] != "" )
		{
			...
			elseif($iColumnIndex==$i_reason){
				$sWhere .= $aColumns[$i_reason]." LIKE '%".mysqli_real_escape_string($db->db_connect_id, $_GET['sSearch_'.$i])."%' OR ";
				$sWhere .= $aColumns[$i_contact]." LIKE '%".mysqli_real_escape_string($db->db_connect_id, $_GET['sSearch_'.$i])."%' ";
			}
			else
		...
		for ( $i=0 ; $i<$max_column ; $i++ ) {
			...
			else
			{
				if($i == 0) {
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = "redirectForm('submissionView.php?submissionID=".$uniqueID."&new=4')";
					...
					if($levelID<=2 || $levelID>=6){
						...
						$imgParam2 = _LBL_EDIT;
						$imgParam3 = "redirectForm('submissionAddEdit.php?submissionID=".$uniqueID."&new=4&resubmit=yes')";
						$activity .= "&nbsp;"._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					}
					/*********/
					$row[] = $activity;
