<?php
	//****************************************************************/
	// filename: checkingNewList.php
	// description: list all list of waiting for checking
	//***************************************************************/
	...
	/*********/
	$dColumn = "s.Submission_Submit_ID,s.Submission_ID,s.Status_ID,s.Type_ID,s.Usr_ID,u.Level_ID, (SELECT count(1) FROM tbl_Submission_Detail sd WHERE s.Submission_ID=sd.Submission_ID ) AS jumlah";
	...
	if($levelID==5){ $dWhere = array("AND"=>"=0"); }
		...
		$dWhere1 = array("AND s.Status_ID"=>"IN (11)");
		$dWhere2 = array();
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					...
					<?php
						foreach($arrayData as $detail){
							...
							$imgParam2 = _LBL_VIEW;
							$imgParam3 = "redirectForm('submissionView.php?submissionID=$uniqueID&new=2')";
							...
							if($statusID==11 && $levelID<=3){
								...
								$imgParam3 = "redirectFormtab('submissionChecking.php?submissionID=$uniqueID&new=2','newtab-$uniqueID')";
								$activity .= "&nbsp;"._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
							}
							/*********/
							$lvlName = "-";
							...
							$sDesc = _get_StrFromCondition("tbl_Submission_Status","Status_Desc". $langdesc,"Status_ID",$statusID);
							/*********/
							echo "
								<tr>
									<td>". $chbx ."</td>
