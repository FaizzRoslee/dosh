<?php
	...
	if(isset($_POST["save"]) || isset($_POST["send"])){ 
		...
		for($i=1;$i<=$noSubMix;$i++){
			...
			$old	 	= isset($_POST["old_$i"])		? $_POST["old_$i"]		: "";
		...
		*/ 
		...
		$(document).on('click', '.cls-delete-row', function(){
			...
			if(row>1){
				...
				// disables the last 'tr' once row=1
                $(this).closest('tr').find('#source_1_1').prop('disabled', true);
			...
			return false;
                ...
                $(document).on('click', '.cls-delete-mix', function(){
			
                
                      
                        $.post("/api/index.php/welcome/padam_mix",({id:$(this).val()}),function(data){
                            
                            //alert(data);
                            //$(this).closest('table').attr('id').remove();
                            location.reload(); 
                        });
			...
			return false;
                ...
                $(document).on('click', '.cls-delete-cem', function(){
			
                        alert($(this).val());
                      
                        $.post("/api/index.php/welcome/padam_cem",({id:$(this).val()}),function(data){
		...
		$(document).on('click', '.cls-show-detail', function(e) {
			...
			$('#' + this.id).popover({
                ...
                <?php 
                $resubmit = isset($_GET['resubmit']) ? $_GET['resubmit'] : 'no';
                ?>
	// /*
	(function($) {
			...
			$.ajax({
				...
				success: function(msg){
					if( $("#chemType").val()==1 ){
						...
						else if(msg.classified==2){
							//alert("Old Record Found , wait until the data is entered by system !");
							$("#selectFrom_PH_"+ msg.row).html( msg.fromPH ).attr("disabled",false);
							$("#selectTo_PH_"+ msg.row).html( msg.toPH ).removeClass("required error");
							$("#selectFrom_HH_"+ msg.row).html( msg.fromHH ).attr("disabled",false);
							$("#selectTo_HH_"+ msg.row).html( msg.toHH ).removeClass("required error");
							$("#selectFrom_EH_"+ msg.row).html( msg.fromEH ).attr("disabled",false);
							$("#selectTo_EH_"+ msg.row).html( msg.toEH ).removeClass("required error");
							
							$("#selectTo_PH_"+ msg.row, "#selectTo_HH_"+ msg.row, "#selectTo_EH_"+ msg.row).closest("td").find("div.error,label.error").show();
							$("#old_"+ msg.row).val(1);
		...
		renumberIngredient = function(tableid){
			...
			$('#' + tableid + ' tbody tr').each(function(){
                ...
                // get value of source field and pass it to the input hidden field
				var value = $('#source_' + itemNo + '_' + i).val();
				// disables source item using their id
				$('#source_' + itemNo + '_' + i).prop('disabled', true);
                // get the hidden input source field
				// $(this).find('input[name^=source_]').attr({ 'name': 'source_' + itemNo + '_' + i, 'id': 'source_' + itemNo + '_' + i });
				$(this).find('input[name^=source_]').attr({ 'name': 'source_' + itemNo + '_' + i, 'id': 'source_' + itemNo + '_' + i, 'value': value});
			...
			$('#' + tableid).closest('table[id^=tbl]').find('input[name^=noIngredient]').val( i );
                ...
                //if (resum == 'yes'){
                    ...
                    //alert($("#noSubMix").val());
                    //$("#noSubMix").attr('disabled','disabled');
                    //$("#noSubMix").hide();
                //}
		...
		<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
			<tr class="contents">
				...
				<td width="20%"><?php
					...
					echo " ". _LBL_QUANTITY .":";
					if ($resubmit == 'yes'){
					$dOthers = array("type"=>"text","readonly" => "readonly","maxlength"=>"2","class"=>"numbersonly ignore text-center",
					...
					echo "<td width='60%' class='nil_submission_addedit_01' >";
			...
			<tr class="contents" id="submission-remark">
				<td class="label nil_submission_addedit_02 "><?= _LBL_REMARK ?></td>
				<td colspan="2" ><?= form_textarea("remark",$remark,array("cols"=>"200","rows"=>"3")); ?></td>
			</tr>
		</table>
