<?php	
	...
	if($_GET['levelID']<5){
		$aColumns = array( ' ','s.Submission_Submit_ID','uc.Company_Name','sl.Level_Name',$colTypeName,$dSubmit,' ','ss.Status_Desc','s.CheckedBy',' ');
		$indexDate = 5;
		$i_datesubmit = 5;
		$i_dateexp = 6;
	}
	else{
		$aColumns = array( ' ','s.Submission_Submit_ID',$colTypeName,$dSubmit,' ','ss.Status_Desc','s.CheckedBy',' ');
		$indexDate = 3;
		$i_datesubmit = 3;
		$i_dateexp = 4;
	}
	...
	*/
	...
	if($levelID==5){ $sWhereCountAll .= " AND = 0 "; }
		...
		$sWhereCountAll .= " AND s.isDeleted = 0 AND s.Status_ID IN (41,42,43,52) ";
