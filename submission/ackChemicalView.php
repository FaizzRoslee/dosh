...
// 				$dataWhereSub1 = array(
// 								"AND s.isDeleted"=>"= 0"
// 								,"AND s.Status_ID"=>"IN (31,32,33,51,41,42,43,52)",
...
//                 old sql query: intepretation of php to sql query
//                 SELECT s.Usr_ID, s.Submission_Submit_ID, sd.Detail_ID, s.Submission_ID 
//                 FROM tbl_Submission s 
//                 LEFT JOIN tbl_Submission_Detail sd ON s.Submission_ID = sd.Submission_ID
// 
//                 AND s.isDeleted = 0 AND s.Status_ID IN (31,32,33,51,41,42,43,52) 
//                 AND Detail_ID IN (SELECT GROUP_CONCAT(Detail_ID separator ',') FROM tbl_Submission_Chemical WHERE Chemical_ID = 51)
                ...
                // linuxhouse_20220608
				// start penambahbaikan - 10_carian hilang - need more info from user - v003: 1
				
				// start debug 1
				// raw sql query 
                // SELECT s.Usr_ID, s.Submission_Submit_ID, sd.Detail_ID, s.Submission_ID FROM tbl_Submission s 
                // JOIN tbl_submission_detail sd ON s.Submission_ID = sd.Submission_ID 
                // JOIN tbl_submission_chemical sc ON sd.Detail_ID = sc.Detail_ID 
                // WHERE s.isDeleted = 0 AND s.Status_ID IN (31,32,33,51,41,42,43,52) AND sc.Chemical_ID=51
                
                // raw sql query explanation
                // 1. select fields from table one and table two
                // 2. from table one
                // 3. join table two using the column(s) table one and table two have in common
                // 4. join table three using the column(s) table table two and table three have in common
                // 5. where results contains selected outputs from table one which have not been deleted and 
                //    selected outputs from table one with status in the range(31,32,33,51,41,42,43,52) and
                //    chemical id from table three is 15
                
                // $dataColumns2 = "s.Usr_ID, s.Submission_Submit_ID, sd.Detail_ID, s.Submission_ID";
                // $dataTable2 = "tbl_Submission s";
                // $dataJoin2 = array("tbl_Submission_Detail sd"=>"s.Submission_ID = sd.Submission_ID");
                // $dataJoin3 = array("tbl_submission_chemical sc"=>"sd.Detail_ID = sc.Detail_ID");
                // $dataJoinResult = array_merge($dataJoin2, $dataJoin3);// php merge 2 arrays
                // $dataWhereSub1 = array(
				//				"AND s.isDeleted"=>"= 0"
				//				,"AND s.Status_ID"=>"IN (31,32,33,51,41,42,43,52)",
				//				"AND sc.Chemical_ID"=>"= ".$chemicalID
				//			);
                ...
                // $dataWhere2 = array_merge($dataWhereSub1,$dataWhereSub2,array("GROUP BY"=>"Usr_ID","ORDER BY"=>"Submission_Submit_ID DESC"));
                // $arrayData2 = _get_arrayData($dataColumns2,$dataTable2,$dataJoinResult,$dataWhere2); 
    
				// $dataWhere2 = array_merge(array("GROUP BY"=>"Usr_ID","ORDER BY"=>"Submission_Submit_ID DESC"));
				// $arrayData2 = _get_arrayData($dataColumns2,$dataTable2,$dataJoin2,$dataWhere2); 
				// end debug 1
				// end penambahbaikan - 10_carian hilang - need more info from user - v003: 1

				
				
				
				// start penambahbaikan - 10_carian hilang - need more info from user - v003: 2
				// start debug 2
				// NOTE:                 
				// selects info which are not deleted but approved
                // SELECT * FROM `tbl_submission_chemical` WHERE isDeleted=0 AND ApprovedDate IS NOT NULL AND Chemical_ID=51
                // SELECT COUNT(*) FROM `tbl_submission_chemical` WHERE isDeleted=0 AND ApprovedDate IS NOT NULL AND Chemical_ID=51
                
				// raw sql query 
                // SELECT s.Usr_ID, s.Submission_Submit_ID, sd.Detail_ID, s.Submission_ID FROM tbl_Submission s 
                // JOIN tbl_submission_detail sd ON s.Submission_ID = sd.Submission_ID 
                // JOIN tbl_submission_chemical sc ON sd.Detail_ID = sc.Detail_ID 
                // WHERE s.isDeleted = 0 AND s.Status_ID IN (31,32,33,51,41,42,43,52) AND sc.ApprovedDate IS NOT NULL
                // AND sc.Chemical_ID=51
                ...
                $dataWhereSub1 = array(
								"AND s.isDeleted"=>"= 0"
								,"AND s.Status_ID"=>"IN (31,32,33,51,41,42,43,52)"
								,"AND sc.ApprovedDate"=>"IS NOT NULL"
								,"AND sc.Chemical_ID"=>"= ".$chemicalID
							);
                ...
                $arrayData2 = _get_arrayData($dataColumns2,$dataTable2,$dataJoinResult,$dataWhere2); 
				// end debug 2
				// end penambahbaikan - 10_carian hilang - need more info from user - v003: 2
