...
<?php
	...
	// filename: announcementNewList_get.php
	// description: get list of the announcements
	//***************************************************************/
	...
	include_once $sys_config["includes_path"]."func_shorten_str.php";// shortens string to n-characters
	//==========================================================
	 ...
	 */
	// $aColumns = array( ' ','Title','StartingDate','EndingDate',' ' );
	$aColumns = array( ' ','Title','Description',' ' );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);

	/* DB table to use */
	$sTable = "tbl_Announcement";
	...
	*/
	 ...
	 */
	// $sWhereCountAll = " WHERE 1 AND isDeleted = 0 ";
    // sample sql
    // SELECT * FROM `tbl_announcement` WHERE (isDeleted=0 AND Active=1) AND (CURDATE() BETWEEN StartingDate AND EndingDate)
    $sWhereCountAll = " WHERE 1 AND (isDeleted=0 AND Active=1) AND (CURDATE() BETWEEN StartingDate AND EndingDate)";
...
//}
	...
	while ( $aRow = $db->sql_fetchrow($rResult) ) {
		$row = array();
		$uniqueID = $aRow['Announcement_ID'];
		
		
		// convert ymd to dmy
        // $aRow["StartingDate"] = func_ymd2dmy($aRow["StartingDate"]);
        // $aRow["EndingDate"] = func_ymd2dmy($aRow["EndingDate"]);
        // get first n words from string followed by ...
        $aRow["Description"] = func_shorten_str($aRow["Description"], 25);
		
		
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" disabled />';
				else{
					$activity = '';
					//edit info====================================================
					$imgParam1 = $sys_config['images_path'].'icons/preview.gif';
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'announcementNewListView.php?stateID='.$uniqueID.'\')';
