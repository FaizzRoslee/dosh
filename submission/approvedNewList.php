<?php
	//****************************************************************/
	// filename: approvedNewList.php
	// description: list of the new submission that has been approved 
	//***************************************************************/
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $("#example").dataTable( {
			...
			"bJQueryUI": true,
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "approvedNewList_get.php?type=<?= $colTypeName ?>&levelID=<?= $levelID ?>&Usr_ID=<?= $Usr_ID ?>&langdesc=<?= $langdesc ?>",
			"aoColumns": [
				...
				<?php } ?>
				{ "sName": "<?= $colTypeName ?>" },
				{ "sName": "<?= $dCount ?>", "bSortable": false, "bSearchable": false },
				{ "sName": "<?= $dSubmit ?>" },
		...
		$("span[class^=img-duplicate-]").live("click",function(){
			// alert( $(this).attr("class") );
			var spclass 	= $(this).attr("class");
			var ar_spclass 	= spclass.split("-");
			var subID		= ar_spclass[2];
			
			if(subID!=""){
				var r = confirm("<?= _LBL_DUPLICATE_SUBMISSION_CONFIRMATION ?>");
				if(r==true){
					$.ajax({
						type: "POST",  
						dataType: "json",
						...
						success: function(msg){
							//alert(msg.newid);
                                                        window.location.replace("/submission/submissionAddEdit.php?submissionID="+msg.newid+"&new=4");
						}
					});
				}
			}
	...
	<?php func_window_open2(_LBL_SUBMISSION ." :: ". _LBL_ACKNOWLEGED); ?>
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					...
					<tfoot>
						<tr>
							...
							<th><?= form_input("search_ctype",_LBL_SEARCH ." ". _LBL_NO_OF_CHEMICAL,$arrOthHid); ?></th>
							<th><?= form_input("search_sdate",_LBL_SEARCH ." ". _LBL_SUBMISSION_DATE,$arrOthHid); ?></th>
