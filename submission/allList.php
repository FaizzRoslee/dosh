<?php
	...
	if(isset($_POST["delete"]) && isset($_POST["checkbox"])){
		...
		foreach($_POST["checkbox"] as $value){
			$status = _get_StrFromCondition("tbl_Submission","Status_ID","Submission_ID",$value);
			$sType	= _get_StrFromCondition("tbl_Submission_Status","Status_Type","Status_Desc",$status);
			if(strtolower($sType)=="new") $statusID = "61";
			else $statusID = "62";
			...
			if($resDel){ 
				...
				$sqlIns = "INSERT INTO tbl_Submission_Record ("
						...
						. " ) VALUE ("
						. " ".quote_smart($msg).",". quote_smart("Delete Submission") .",".quote_smart($updBy).",NOW(),".quote_smart($statusID).",".quote_smart($value).""
						. " )";
		...
		<div id="container">
			<div class="demo_jui">
					...
					<tr>
						...
						<td width="40%" align="right"><?php
							...
							if($levelID>=6 || $levelID<=2){
								$strBtn .= form_button("add",_LBL_ADD ." ". _LBL_SUBMISSION,array("type"=>"button","onClick"=>"redirectForm('submissionAddEdit.php?new=2')"));
							}
							if($levelID==1){ 
								if(!empty($strBtn)) $strBtn .= "&nbsp;";
								$strBtn .= form_button("delete",_LBL_DELETE,array("type"=>"submit")); 
							}
							echo $strBtn;
						?></td>
					</tr>
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							<th><input type="checkbox" name="checkAll" id="checkAll" <?= (($levelID==1)?"":"disabled") ?> /></th>
