<?php
	//****************************************************************/
	// filename: renewAddEdit.php
	// description: add/edit the details of the renew submission
	//****************************************************************/
	...
	//===================
	if($new==14) $formBack = 'rejectedRenewList.php';
	elseif($new==13) $formBack = 'approvedRenewList.php';
	...
	if(isset($_POST['save']) || isset($_POST['send'])){
		...
		if($submissionID!=0){ //===================================================================================================
			...
			if($statusID==3||$statusID==4||$statusID==41||$statusID==42){
				$sqlUpd	= "UPDATE tbl_Submission SET"
						//. " Submission_Submit_ID = ".quote_smart(_get_StrFromCondition('tbl_Submission','Submission_Submit_ID','Submission_ID',$submissionID)).","
						. " Type_ID = ".quote_smart($chemType).","
						. " Submission_Remark = ".quote_smart($remark).",";
				if($statusID==41||$statusID==42)
					$sqlUpd	.= " Status_ID = '42',";
				$sqlUpd	.= " Disclaimer = ".quote_smart($disclaimer).", UpdateBy = ".quote_smart($Usr_ID).", UpdateDate = NOW()" 
						. " WHERE Submission_ID = ".quote_smart($submissionID)." LIMIT 1";
				//echo $sqlUpd.'<br />';
				$resIns	= $db->sql_query($sqlUpd,END_TRANSACTION) or die(print_r($db->sql_error()));
				//==========================================================
				if($resIns){ $msg1 = 'Update Successfully'; }
				else{ $msg1 = 'Update Unsuccessfully'; }
				func_add_audittrail($Usr_ID,$msg1.' ['.$submissionID.']','Submission-Renew','tbl_Submission,'.$submissionID);
				//==========================================================
				$detailIDs = '';
				for($i=1;$i<=$noSubMix;$i++){
					...
					if($chkbx!=0){
						if($detailID==''){
							...
							if($chemType==2) //if mixture
								$sqlUpd .= " Detail_ProductName = ".quote_smart($productName).",";
							$sqlUpd .= " Detail_ID_Number = ".quote_smart($idNo).","
									. " Form_ID = ".quote_smart($physicalForm).","
									. " Category_ID_ph = ".quote_smart($phCategory_new).","
									. " Category_ID_hh = ".quote_smart($hhCategory_new).","
									. " Category_ID_eh = ".quote_smart($ehCategory_new).",";
							if($levelID!=7) {
								if($totalSupply!='') $sqlUpd .= " Detail_TotalSupply = ".quote_smart($totalSupply).",";
								// $sqlUpd .= " Detail_EntityTo = ".quote_smart($entityTo_new).",";
							}
							if($levelID!=6) {
								if($totalUse!='') $sqlUpd .= " Detail_TotalUse = ".quote_smart($totalUse).",";
								// $sqlUpd .= " Detail_EntityFrom = ".quote_smart($entityFrom_new).",";
							}
							$sqlUpd	.= " Submission_ID = ".quote_smart($submissionID).", UpdateDate = NOW()"
									. " WHERE Detail_ID = ".quote_smart($detailID)." LIMIT 1";
						...
						for($j=1;$j<=$noIngredient;$j++){
							...
							if($chemicalName!=''){
								...
								if($subChemID==''){
									...
									if($chemicalID==''){
										$sqlUpd .= " Sub_Chemical_Name = ".quote_smart($chemicalName).",";
										$sqlUpd .= " Sub_Chemical_CAS = ".quote_smart($casNo).",";
									}
									if($composition!='') $sqlUpd .= " Sub_Chemical_Composition = ".quote_smart($composition).",";
									//if($nolonger!='') 
										$sqlUpd .= " StopDate = ". (($nolonger!='')?quote_smart($nolonger):"NULL") .",";
									$sqlUpd .= " Sub_Chemical_Source = ".quote_smart($source).",";
									$sqlUpd .= " Chemical_ID = ".quote_smart($chemicalID).","
											. " Detail_ID = ".quote_smart($detailID).","
											. " UpdateDate = NOW()"
											. " WHERE Sub_Chemical_ID = ".quote_smart($subChemID)." LIMIT 1";
									//echo $sqlUpd.'<br />';
									$resUpd	= $db->sql_query($sqlUpd,END_TRANSACTION) or die(print_r($db->sql_error()));
									//==========================================================
				...
				if($statusID==41||$statusID==42){
					$menu = 13; 
					$file = "approvedRenewList.php";
					$msg = 'Update acknowledge submission'; 
					//==========================================================
					$sqlIns = "INSERT INTO tbl_Submission_Record ("
							...
							. " ) VALUE ("
							. " ".quote_smart($msg).",".quote_smart($remark).",".quote_smart($Usr_ID).",NOW(),32,".quote_smart($submissionID).")";
					$resIns	= $db->sql_query($sqlIns,END_TRANSACTION) or die(print_r($db->sql_error()));
					//==========================================================
					func_add_audittrail($Usr_ID,$msg.' ['.$submissionID.']','Submission-Renew','tbl_Submission,'.$submissionID);
					//==========================================================
					// func send emel notification update
					$staffApprv = _get_StrFromCondition('tbl_Submission','ApprovedBy','Submission_ID',$submissionID);
					$emelApprv	= _get_StrFromCondition('sys_User_Staff','User_Email','Usr_ID',$staffApprv);
					$content = _get_Notification3($emelApprv,$submissionID);
					if(is_array($content)){
						if($content['email']!=''){
	...
	//============================================================
	if($statusID==31||$statusID==52) $addEdit = _LBL_ADD;
...
?>
		...
		<tr class="contents">
			...
			<td width="80%"><label><?php
				$note = _get_StrFromManyConditions('tbl_Submission_Record','Sub_Record_Remark',
								array('AND Sub_Record_Status'=>'= 4','AND Submission_ID'=>'= '.quote_smart($submissionID),'ORDER BY'=>'Sub_Record_ID DESC')
							); 
	...
	<?php		
		if($chem_type==1 && $noRow>0){
			...
			foreach($arrayData as $detail){
				...
				echo form_input($param1,$param2,$param3);
				//======================
				if($statusID<10){ $str_onClick = "getChemID('".$i."','1');"; }
				else{ $str_onClick = ""; }
				//======================
				$classified = 0;
				$arr_catIDs = array();
				...
				if(in_array($statusID,array('31','32','41','42'))){
					$labelTitle = '<label><input type="checkbox" name="'.$chk_name.'" id="'.$chk_name.'" value="1" '.$chk_checked
								. ' class="cl_chkbx" />'. _LBL_NO_OF_SUBSTANCE .' '.$i .'</label>'
								. '';
				}else{
					$labelTitle = '<label><input type="checkbox" name="'.$chk_name.'" id="'.$chk_name.'" value="1" '.$chk_checked
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%" id="<?= $tblID ?>">
		...
		<tr class="contents">
			...
			<td colspan="3" valign="middle"><?php
				...
				$param7 = '';
				if($classified!=0){ $param9 = $arr_catIDs; }
				else{
					if($chemID==$newchemID){ 
	...
	<?php
		...
		elseif($chem_type==2 && $noRow>0){
			...
			foreach($arrayData as $detail){
				...
				if(in_array($statusID,array('31','32','41','42'))){
					$labelTitle = '<label><input type="checkbox" name="'.$chk_name.'" id="'.$chk_name.'" value="1" '.$chk_checked
								. ' class="cl_chkbx" />'. _LBL_NO_OF_MIXTURE .' '.$i .'</label>';
				}else{
					$labelTitle = '<label><input type="checkbox" name="'.$chk_name.'" id="'.$chk_name.'" value="1" '.$chk_checked
			...
			<td colspan="4"><table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
				...
				<?php
					...
					foreach($arrayData as $detail){
						...
						echo form_input($param1,$param2,$param3);
						//======================
						if($statusID<10){ $str_onClick = "getChemID('".$i."','".$j."',true);"; $str_title="title='". _LBL_JUST_CLICK_ADDEDIT ."'"; }
				...
				<tr class="contents" style="vertical-align:middle">
					...
					<td align="center"><?php
						...
						$param2 = isset($_POST[$param1])?$_POST[$param1]:$casNo;
						$param3 = array('type'=>'text', 'readonly'=>'readonly',
										'style'=>'width:96%','onClick'=>$str_onClick);
						echo '<span class="ignoreValidate">'.form_input($param1,$param2,$param3).'</span>'; 
					?></td>
					<td align="center"><?php
						/*
				...
				<tr class="contents" style="vertical-align:middle">
					...
					<td><?php
						$param1 = 'source_'.$i.'_'.$j;
						$param2 = $arrSource;
						$param3 = isset($_POST[$param1])?$_POST[$param1]:0;
						$param4 = '<br />';
						echo form_radio($param1,$param2,$param3,$param4,'');
						echo '<div class="error"><label for="'.$param1.'" class="error">'. _LBL_REQUIRED_FIELD .'</label></div>';
					?></td>
			...
			<td colspan="4"><table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
				...
				<tr class="contents" style="vertical-align:middle">
					...
					<td align="center"><?php
						...
						$param2 = isset($_POST[$param1])?$_POST[$param1]:'';
						$param3 = array('type'=>'text', 'readonly'=>'readonly',
										'style'=>'width:98%','onClick'=>"getChemID('".$i."','".$j."',true);");
						echo form_input($param1,$param2,$param3); 
						echo '<div class="error"><label for="'.$param1.'" class="error">'. _LBL_REQUIRED_FIELD .'</label></div>';
					?></td>
					<td align="center"><?php
						...
						$param2 = isset($_POST[$param1])?$_POST[$param1]:'';
						$param3 = array('type'=>'text', 'readonly'=>'readonly',
										'style'=>'width:97%','onClick'=>"getChemID('".$i."','".$j."',true);");
						echo '<span class="ignoreValidate">'.form_input($param1,$param2,$param3).'</span>';
					?></td>
					<td align="center"><?php
						/*
	...
	<?php 
		...
		if($noRow>0){
			if($submissionID!=0 && !in_array($statusID,array('31','32','41','42'))){
	?>
	<br />
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
		...
		<tr class="contents">
			<td width="5%" align="center" valign="middle"><?php
				$param1 = 'disclaimer';
				$param2 = isset($_POST[$param1])?1:$disclaimer;
				$dischecked = $param2;
				if(in_array($statusID,array('31','32','41','42'))){
					$param3 = array('type'=>'checkbox','onClick'=>'func_check(this);','checked'=>'checked','readonly'=>'readonly');
				}else{
					if($dischecked==0)
	...
	<?php
			...
			else{ echo form_input('disclaimer',1,array('type'=>'hidden')); }
	?>
	<br />
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		<tr class="contents">
			<td align="center"><?php
				if($userLevelID<=2 || $userLevelID>=6){
					...
					if($statusID==3||$statusID==4){
						...
						echo '<span id="spanSubmit" style="display:none"></span>';
					}
				}
				$param1 = 'cancel';
				$param2 = _LBL_CANCEL;
