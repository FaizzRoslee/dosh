...
?>	
	...
	<?php		
		if($noRow>0){
			...
			$dataColumns = "Detail_ID,Detail_ProductName,Detail_ID_Number,Form_ID,Category_ID_ph,Category_ID_hh,Category_ID_eh,"
						. "Detail_TotalSupply,Detail_TotalUse,ApprovedDate,RecordDate,old";
	...
	<?php 
	if ($old == 1) {
		$class = ' <img src="/images/check.png" width="15" alt="old submit">';
	}
	else{
		$class = "";
	}
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		...
		<tr class="contents">
			<td width="20%" class="label"><?= _LBL_TRADE_PRODUCT ?><?=$class?> </td>
		...
		<!--
			Greentick for hazard classification: Criteria
			1. Only DOHS Staff
			2. Only chemical type is substance
			3. Health Hazard Classification Submission must meet the requirements with Hazard Checking
		-->
		<?php
			if($userLevelID<=5 && $chemType==1) {
				$greenTick = '<img src="/images/check.png" width="15">';

				if(is_array($hhCategory)){
					foreach($hhCategory as $value){
						if($value!=''){
							echo _get_StrFromCondition('tbl_Hazard_Category',
								"CONCAT_WS(': ',(SELECT Class_Name FROM tbl_Hazard_Classification WHERE Class_ID = tbl_Hazard_Category.Class_ID LIMIT 1),Category_Name)", 'Category_ID',$value);
							echo '</a>;<br />';
						}
					}
				}
		...
		<tr class="contents">
			<td class="label">
				<?= _LBL_HAZARD_CLASSIFICATION ?> <?=$greenTick?>
			</td>
			...
			<td colspan="3" align="center"><table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
				...
				<!-- Physical Hazard -->
				<tr class="contents">
					<td class="label nil_submission_addedit_03" width="30%"><?= _LBL_PHY_HAZARD_CLASS ?></td>
