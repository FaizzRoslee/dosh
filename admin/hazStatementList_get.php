<?php
	//****************************************************************/
	// filename: hazStatementList_get.php
	// description: get list of the hazard statement
	//***************************************************************/
	...
	$sIndexColumn = "hs.Statement_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( ' ','ht.Type_Name','hs.Statement_Code','hs.Statement_Desc','hs.Active',' ' );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);

	/* DB table to use */
	$sTable = "tbl_Hazard_Statement hs";
	...
	* Joining
	*/
	$sJoin = ' LEFT JOIN tbl_Hazard_Type ht ON hs.Type_ID = ht.Type_ID';
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND hs.isDeleted = 0";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = 'redirectForm(\'hazStatementAddEdit.php?statementID='.$uniqueID.'\')';
