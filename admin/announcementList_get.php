<?php
	...
	// filename: announcementList_get.php
	 ...
	 */
	$aColumns = array( ' ','Title','Description','StartingDate','EndingDate','Active',' ' );
	$aColumns2 = $aColumns;
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = 'redirectForm(\'announcementListAddEdit.php?stateID='.$uniqueID.'\')';
