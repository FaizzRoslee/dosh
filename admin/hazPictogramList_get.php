<?php
	//****************************************************************/
	// filename: hazPictogramList_get.php
	// description: get list of the hazard pictogram
	//***************************************************************/
	...
	$sIndexColumn = "Pictogram_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( ' ','ht.Type_Name','hp.Pictogram_Desc','hp.Pictogram_Url','hp.Active',' ' );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn);

	/* DB table to use */
	$sTable = "tbl_Hazard_Pictogram hp";
	...
	* Joining
	*/
	$sJoin = ' LEFT OUTER JOIN tbl_Hazard_Type ht ON hp.Type_ID = ht.Type_ID ';
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" />';
					...
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = 'redirectForm(\'hazPictogramAddEdit.php?pictogramID='.$uniqueID.'\')';
				...
				}elseif($col[$countCol-1]=='Pictogram_Url'){
					$fileName = explode('/',$aRow[ $col[$countCol-1] ]);
					$countArr = count($fileName);
					
					$row[] = '<img src="'.$aRow[ $col[$countCol-1] ].'" width="50" height="50" />';
				}else{
