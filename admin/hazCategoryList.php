<?php
	...
	if(isset($_POST["checkbox"]) && (isset($_POST["active"]) || isset($_POST["inactive"]))){
		...
		foreach($_POST["checkbox"] as $value){
			$sqlDel = "UPDATE tbl_Hazard_Category SET"
					. " Active = ".quote_smart($active).","
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Category_ID = ".quote_smart($value)." LIMIT 1"; //set active==0 instead delete from table;
			//echo $sqlDel;
			...
			/*********/
			func_add_audittrail($updBy,$msg." [".$value."]","Admin-Hazard Category","tbl_Hazard_Category,".$value);
		}
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $('#example').dataTable( {
			...
			"sPaginationType": "full_numbers",
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "hazCategoryList_get.php",
			"aoColumns": [
				{ "sName": " ", "bSortable": false},
				{ "sName": "ht.Type_Name" },
				{ "sName": "hc.Class_Name" },
				{ "sName": "hcat.Category_Name" },
				{ "sName": "hs.Signal_Name" },
				{ "sName": "hcat.Active" },
				{ "sName": " ", "bSortable": false }
 			],
			"aaSorting": [[ 1, 'asc' ]],
	...
	<?php func_window_open2(_LBL_HAZARD_CATEGORY); ?>
				...
				<table style="width:100%">
					<tr>
						...
						<td width="40%" align="right"><?php
							/*********/
							$dOthers = array("type"=>"button","onClick"=>"redirectForm('hazCategoryAddEdit.php')");
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							...
							<th width="20%"><?= _LBL_HAZARD_CLASSIFICATION ?></th>
							<th width="25%"><?= _LBL_HAZARD_CATEGORY ?></th>
							<th width="12%"><?= _LBL_SIGNAL_WORD ?></th>
					...
					<tfoot>
						<tr>
							...
							<th><input type="text" name="search_class" value="<?= _LBL_SEARCH .' '. _LBL_HAZARD_CLASSIFICATION ?>" class="search_init" /></th>
							<th><input type="text" name="search_cat" value="<?= _LBL_SEARCH .' '. _LBL_HAZARD_CATEGORY ?>" class="search_init" /></th>
							<th><input type="text" name="search_signal" value="<?= _LBL_SEARCH .' '. _LBL_SIGNAL_WORD ?>" class="search_init" /></th>
