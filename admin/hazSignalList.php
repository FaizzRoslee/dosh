<?php
	//****************************************************************/
	// filename: hazSignalList.php
	// description: list of the hazard signal
	//***************************************************************/
	...
	if(isset($_POST['checkbox']) && (isset($_POST['active'])||isset($_POST['inactive']))){
		...
		foreach($_POST['checkbox'] as $value){
			$sqlDel = "UPDATE tbl_Hazard_Signal SET"
					. " Active = ".quote_smart($active).","
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Signal_ID = ".quote_smart($value)." LIMIT 1"; //set active==0 instead delete from table;
			...
			//==========================================================
			func_add_audittrail($updBy,$msg.' ['.$value.']','Admin-Hazard Signal','tbl_Hazard_Signal'.$value);
		}
		//=================================================
	...
	if(isset($_POST['checkbox']) && isset($_POST['delete'])){
		...
		foreach($_POST['checkbox'] as $value){
			$sqlDel = "UPDATE tbl_Hazard_Signal SET"
					...
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Signal_ID = ".quote_smart($value)." LIMIT 1";
			$resDel = $db->sql_query($sqlDel,END_TRANSACTION) or die(print_r($db->sql_error()));
			//=================================================
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $('#example').dataTable( {
			...
			"sPaginationType": "full_numbers",
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "hazSignalList_get.php",
			"aoColumns": [
	...
	<?php func_window_open2(_LBL_SIGNAL_WORD); ?>
				...
				<table style="width:100%">
					<tr>
						...
						<td width="40%" align="right"><?php
							...
							$param1 = 'add';
							$param2 = _LBL_ADD;
							$param3 = array('type'=>'button','onClick'=>'redirectForm(\'hazSignalAddEdit.php\')');
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					...
					<tfoot>
						<tr>
							...
							<th><input type="text" name="search_type" value="<?= _LBL_SEARCH .' '. _LBL_SIGNAL ?>" class="search_init" /></th>
