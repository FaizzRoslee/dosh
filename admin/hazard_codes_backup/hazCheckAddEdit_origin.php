	...
	if(isset($_POST["save"])){
					...
					if(is_array($ex_str)){
						...
						foreach($ex_str as $value){
							if(trim($value)!=""){
								...
								$strStatement .= _get_StrFromCondition("tbl_Hazard_Statement","Statement_ID","Statement_Code",trim($value));
							}
						}
					}
					/*********/
		...
		if($categoryID!=0){
			...
			$sqlIns	= "INSERT INTO tbl_Hazard_Codes ("
					. " Chemical_Name,CAS_No,Pictogram_ID,Signal_ID,"
					...
					. " UpdateBy, UpdateDate"
					. " ) VALUES ("
					. " ".quote_smart($chemicalName).",".quote_smart($casNo).",".quote_smart($pictogramID).",".quote_smart($signalID).","
...
?>
	...
	<table border="0" class="contents" cellspacing="1" cellpadding="3" width="100%">
		...
		<?php
			$arrPreStatement = _get_arraySelect("tbl_Hazard_Type","Type_ID","Type_Name",array("AND Active"=>"= 1"),"Type_ID");
		...
		<tr class="contents">
			...
			<td><a href="#" class="cls-add" id="pr-<?= $pID ?>"><?= _LBL_ADD ?></a><br /><?php
				...
				echo 	'<thead>';
				echo 	'<tr class="contents"><td width="5%" class="label" align="center">'. _LBL_NUMBER .'</td><td width="85%" class="label">'. _LBL_CLASSIFICATION_CODE .'</td><td width="10%" class="label">'. _LBL_DELETE .'</td></tr>';
				echo 	'</thead>';
				echo 	'<tbody>';
				...
				foreach($existingData as $value){
					if($value!=""){
						...
						foreach($ex_value as $value2){
							if($value2!="")
								$new_value .= _get_StrFromCondition("tbl_Hazard_Statement","Statement_Code","Statement_ID",trim($value2)).", ";
						}
						/*********/
