<?php
	 ...
	 */
	$aColumns = array( " ","c.Chemical_CAS","c.Chemical_Name","hc.Active"," " );
	$aColumns2 = $aColumns;
	...
	*/
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND c.isActive = 1 AND c.isDeleted = 0 AND hc.Active = 1 AND c.isNew = 0 ";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				...
				elseif($key==($countArr-1)){
					...
					$imgParam2 = _LBL_VIEW;
					$imgParam3 = 'redirectForm(\'hazCheckView.php?chemicalID='.$uniqueID.'\')';
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//====================================================
					if($userLevelID<=2){
						$imgParam1 = $sys_config['images_path'].'icons/edit.gif';
						$imgParam2 = _LBL_EDIT;
						$imgParam3 = 'redirectForm(\'hazCheckAddEdit.php?chemicalID='.$uniqueID.'\')';
						$activity .= '&nbsp;'._get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					}
					//====================================================
