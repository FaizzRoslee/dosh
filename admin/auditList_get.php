<?php
	//****************************************************************/
	// filename: hazClassList_get.php [physical,health,environmental]
	...
	$sIndexColumn = "at.AuditTrail_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( " ","at.AuditTrail_Mod","at.AuditTrail_Desc"," ","at.IP_Address","at.RecordDate" );
	$aColumns2 = $aColumns;
	array_push($aColumns2,$sIndexColumn,'at.RecordBy');

	/* DB table to use */
	$sTable = "sys_AuditTrail at";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" disabled />';
					...
					$str = "";
					/*********/
					$str .= _get_User($aRow["RecordBy"]);
					$str .= "<br />";
					$str .= "[". func_ymd2dmytime($aRow["RecordDate"]) ."]";
					/*********/
					$row[] = $str;
				}
			}else{
