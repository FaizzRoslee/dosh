<?php
	...
	// description: get list of the hazard checking
	//***************************************************************/
	 ...
	 */
	$aColumns = array( " ","hcode.Chemical_Name","hcode.CAS_No","hcode.Active"," " );
	$aColumns2 = $aColumns;
	...
	* Joining
	*/
// 	$sJoin = ' RIGHT JOIN tbl_Hazard_Codes hc ON c.Chemical_ID = hc.Chemical_ID';
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND hcode.isDeleted = 0 ";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				...
				if($col[$countCol-1]=='Active'){
					...
					$row[] = $category;
				}elseif($col[$countCol-1]=='Statement_ID'){
					$det = $aRow[ $col[$countCol-1] ];
					$statement = '';
					if($det!=''){
						$ex_det = explode(',',$det);
