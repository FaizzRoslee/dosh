<?php
	...
	if(isset($_POST['checkbox']) && (isset($_POST['active'])||isset($_POST['inactive']))){
		...
		foreach($_POST['checkbox'] as $value){
			$sqlDel = "UPDATE tbl_Hazard_Classification SET"
					. " Active = ".quote_smart($active).","
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Class_ID = ".quote_smart($value)." LIMIT 1"; //set active==0 instead delete from table;
			...
			//==========================================================
			func_add_audittrail($updBy,$msg.' ['.$value.']','Admin-Hazard Classification','tbl_Hazard_Classification'.$value);
		}
	...
	if(isset($_POST['checkbox']) && isset($_POST['delete'])){
		...
		foreach($_POST['checkbox'] as $value){
			$sqlDel = "UPDATE tbl_Hazard_Classification SET"
					...
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE Class_ID = ".quote_smart($value)." LIMIT 1";
			$resDel = $db->sql_query($sqlDel,END_TRANSACTION) or die(print_r($db->sql_error()));
			//=================================================
...
?>
	...
	$(document).ready(function() {
		...
		var oTable = $('#example').dataTable( {
			...
			"sPaginationType": "full_numbers",
 			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "hazClassList_get.php",
			"aoColumns": [
	...
	<?php func_window_open2(_LBL_HAZARD_CLASSIFICATION); ?>
				...
				<table style="width:100%">
					<tr>
						...
						<td width="40%" align="right"><?php
							...
							$param1 = 'add';
							$param2 = _LBL_ADD;
							$param3 = array('type'=>'button','onClick'=>'redirectForm(\'hazClassAddEdit.php\')');
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							...
							<th width="25%"><?= _LBL_TYPE_NAME ?></th>
							<th width="46%"><?= _LBL_CLASSIFICATION ?></th>
					...
					<tfoot>
						<tr>
							...
							<th><input type="text" name="search_name" value="<?= _LBL_SEARCH .' '. _LBL_CLASSIFICATION ?>" class="search_init" /></th>
