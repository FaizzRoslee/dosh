<?php
	//****************************************************************/
	// filename: stateList.php
	// description: list of the state
	//***************************************************************/
	...
	if(isset($_POST['checkbox']) && (isset($_POST['active']) || isset($_POST['inactive']))){
		...
		foreach($_POST['checkbox'] as $value){
			$sqlDel = "UPDATE sys_State SET"
					. " Active = ".quote_smart($active).","
					. " UpdateBy = ".quote_smart($updBy).","
					. " UpdateDate = NOW()"
					. " WHERE State_ID = ".quote_smart($value)." LIMIT 1"; //set active==0 instead delete from table;
			...
			//==========================================================
			func_add_audittrail($updBy,$msg.' ['.$value.']','Admin-State','sys_State,'.$value);
		}
...
?>
	...
	<?php func_window_open2(_LBL_STATE_LIST); ?>
				...
				<table style="width:100%">
					<tr>
						...
						<td width="40%" align="right"><?php
							...
							$param1 = 'add';
							$param2 = _LBL_ADD;
							$param3 = array('type'=>'button','onClick'=>'redirectForm(\'stateAddEdit.php\')');
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
					<thead>
						<tr>
							...
							<th width="16%"><?= _LBL_STATE_CODE ?></th>
							<th width="55%"><?= _LBL_STATE_NAME ?></th>
					...
					<tfoot>
						<tr>
							...
							<th><input type="text" name="search_staffid" value="<?= _LBL_SEARCH .' '. _LBL_STATE_CODE ?>" class="search_init" /></th>
							<th><input type="text" name="search_name" value="<?= _LBL_SEARCH .' '. _LBL_STATE_NAME ?>" class="search_init" /></th>
