<?php
	//****************************************************************/
	// filename: stateList_get.php
	// description: get list of the state
	//***************************************************************/
	...
	$sIndexColumn = "Info_ID";
	
	/* Array of database columns which should be read and sent back to DataTables. Use a space where
	 * you want to insert a non-database field (for example a counter or static image)
	 */
	$aColumns = array( ' ','Info_For',' ' );
	$aColumns2 = $aColumns;
	...
	*/
	 ...
	 */
	$sWhereCountAll = " WHERE 1 AND Active = 1";
		...
		foreach($aColumns as $key => $detail){
			if($detail==' '){
				if($key==0) $row[] = '&nbsp;<input type="checkbox" name="checkbox[]" value="'.$uniqueID.'" disabled />';
					...
					$imgParam2 = _LBL_VIEW;
					if ($uniqueID == 40) {
					$imgParam3 = 'redirectForm(\'splash.php?infoID='.$uniqueID.'\')';	
					}
					else { 
					$imgParam3 = 'redirectForm(\'infopageView.php?infoID='.$uniqueID.'\')';
					}
					$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//edit info====================================================
					$imgParam1 = $sys_config['images_path'].'icons/edit.gif';
					$imgParam2 = _LBL_EDIT;
					$imgParam3 = 'redirectForm(\'infopageAddEdit.php?infoID='.$uniqueID.'\')';
					//$activity .= _get_imagebutton($imgParam1,$imgParam2,$imgParam3);
					//edit info====================================================
					$row[] = $activity;
				}
