<?php
	...
	function _get_datalama($tbl="",$cid="",$id=""){
		//function baru di cipta oleh kakitangan stm dosh
		global $db;
		$val = "";
		$sql = 'select '.$tbl.' FROM (select sd.Submission_ID AS Submission_ID,sd.Detail_ProductName AS Detail_ProductName,sd.Category_ID_ph AS Category_ID_ph,sd.RecordDate AS RecordDate,sd.Category_ID_eh AS Category_ID_eh,sd.Category_ID_hh AS Category_ID_hh,(select s.Usr_ID from jkkp_einventory.tbl_submission s where (s.Submission_ID = sd.Submission_ID)) AS user,(select c.Chemical_ID from jkkp_einventory.tbl_chemical c where (c.Chemical_ID = sc.Chemical_ID)) AS Chemical_ID from (jkkp_einventory.tbl_submission_detail sd left join jkkp_einventory.tbl_submission_chemical sc on((sd.Detail_ID = sc.Detail_ID)))) As data WHERE Chemical_ID="'.$cid.'" AND user="'.$id.'" ORDER BY  RecordDate DESC  LIMIT 1 ';
		//$sql = "SELECT ".$tbl." FROM (".$ssql.") WHERE user =".$uid." AND Chemical_ID =".$cid." WHERE 1";
		//$sql = "SELECT ".$tbl." FROM view_subcge_dosh WHERE Chemical_ID=".$cid." AND user=".$id." ORDER BY  RecordDate DESC  LIMIT 1";
	...
	function _get_imagedoc($titleAlt='',$func='',$others=array()){
        ...
        function _get_imagedoc2($titleAlt='',$func='',$others=array()){
		global $sys_config;
		
		//$imgBtn = "<img src=\"".$sys_config['images_path']."icons/download.gif\" title=\"".$titleAlt."\" alt=\"".$titleAlt."\""
				//. " style=\"cursor:hand;cursor:pointer;\" onClick=\"".$func."\"";
		$imgBtn = "<a href='".$func."'>".$titleAlt."</a>";
		//if(is_array($others)){
			//foreach($others as $attribute => $detail)
				//$imgBtn .= ' '.$attribute.'="'.$detail.'"';
		//}
	...
	function func_fn_escape($str_escape='') {
		if(!empty($str_escape)) {
			...
			$str_escape = str_replace("�", "&szlig;", $str_escape);
	...
	function getComposition($composition="",$col="value"){
		...
		return number_format((float)$pComp,3,".",",");
	}
