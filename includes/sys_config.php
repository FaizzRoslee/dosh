<?php
    ...
    Update 20211101 : LinuxHouse is currently reponsible for the maintenance of this CIMS
                      ...
                      With regards to this config file, pls note that not all params 
                      are present here and some are not actually in use 
	...
	**********************************************************************************/
	...
	$sys_config["org_fax_no"]				    =		"+603-8890 1315";
	...
	$sys_config["submit_end"]	  = date("Y") ."-06-30";
...
if( 1 === 2 ){    // linuxhouse_debug_20211104  

    
    echo "<pre>";    echo "/var/www/html/includes/sys_config.php";       echo "</pre>"; 
    echo "<pre>";    print_r($_SESSION["user"]);                         echo "</pre>";  
    
    trigger_error("Language Path = ".$sys_config["languages_path"], E_USER_NOTICE);
